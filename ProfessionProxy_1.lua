...
function ProfessionProxy:ParseProfession()
	...
	if(rootClass) then
		...
		error(string.format("?????????????????????????????????id %d????????????",rootID))
	end
end
	...
	if previousId then
		...
		if previousData.MaxPeak~=nil then
			...
			if doublePreId then
				...
				helplog("GetThisIdJiuZhiTiaoJianLevel(id) ???????????????")
			end	
		else
...
function ProfessionProxy:PurchaseFunc(id)
	...
	if GameConfig.ProfessionExchangeTicket== nil then
		if ApplicationInfo.IsRunOnEditor() then
			MsgManager.FloatMsg(nil, "??????????????????  GameConfig.ProfessionExchangeTicket????????????")
		end	
		do return end
	end	
