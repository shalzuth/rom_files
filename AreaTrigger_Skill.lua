...
function AreaTrigger_Skill:SkillTransport_ResumeSyncMove(nextFrame)
	-- helplog("?????????????????? SkillTransport_ResumeSyncMove",self.skill_transport_resumeSyncMoveFlag,Time.time)
	if(nextFrame) then
		--???????????????????????????ai???????????????????????????
...
function AreaTrigger_Skill:EnterSkillTransport(trigger)
	...
	-- helplog("stop???????????? SkillTransport_ResumeSyncMove",self.skill_transport_resumeSyncMoveFlag)
