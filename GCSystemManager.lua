...
function GCSystemManager:InitData()
	--gc?????????????????????sampleTimeGapCount???????????????????????????limitAverageGapTime,?????????LimitLevel_1???LimitLevel_2
...
function GCSystemManager:_SetTime()
	...
	if(average <= self.limitAverageGapTime) then
		...
		LogUtility.InfoFormat("gc ????????????gc????????????: {0} ms,????????????gc ???????????? 1:{1} kb 2:{2} kb",average,LimitLevel_1,LimitLevel_2)
		self:_ResetTimeGap()
	end
end
--??????
