...
function ShopMallProxy:InitExchangeBuyTypes()
	...
	self.exchangeBuyParentTypes = {}	--??????Table_ItemTypeAdventureLog??????data
	...
	self.exchangeBuyChildTypes = {}	--self.exchangeBuyChildTypes[?????????id] = {?????????data}?????????Table_ItemTypeAdventureLog??????data
...
function ShopMallProxy:InitExchangeBuyClassify()
	self.exchangeBuyClassify = {}	--self.exchangeBuyClassify[?????????id????????????id] = {??????item id}?????????Table_ItemType??????id
