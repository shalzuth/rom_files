...
function ChangeZoneView:XDEUpdateInfo()
	if self.contentInput.value == "" then
		...
		local num = OverseaHostHelper:ZoneInfoToNum(self.itemTabs.value,self.contentInput.value)
		self:UpdateCost(num)
		self:UpdateTip()
...
function ChangeZoneView:InitXDE()
	--todo xde
...
function ChangeZoneView:UpdateCurrentZone()
	--todo xde
	self.curZoneInfo = OverseaHostHelper:GetCurZoneInfo();
	if self.curZoneInfo ~= nil then
		self.currentZone.text = self.curZoneInfo.id
		self.itemTabs.value = self.curZoneInfo.name
	end
end
...
--	local num = ChangeZoneProxy.Instance:ZoneStringToNum(value)
	-- todo xde
	self.isBusy = false
	local num = OverseaHostHelper:ZoneInfoToNum(self.itemTabs.value,value)
...
--		self.contentInput.value = ChangeZoneProxy.Instance:ZoneNumToString(self.currentZoneId)
		local zoneInfo = OverseaHostHelper:GetZoneInfo(self.currentZoneId)
		if zoneInfo ~=nil then
			self.contentInput.value = tostring(zoneInfo.id)
			self.itemTabs.value = zoneInfo.name
...
function ChangeZoneView:ClickChangeBtn()
	...
	-- todo xde
	if value == self.currentZone.text and self.itemTabs.value == self.curZoneInfo.name then
