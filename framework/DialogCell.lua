...
function DialogCell:SetData(dialogData, params)
	...
	if(dialogData)then
		...
		if(context == "")then
			...
			-- todo xde start ajust 埋点：转职任务结束，见Table_Dialog
...
ReplaceParam_FuncMap[ DialogParamType_StoragePrice ] = function ()
	...
	if(free_actid)then
		...
		local running = FunctionActivity.Me():GetActivityData( free_actid ) ~= nil;
		if(running)then
			return 0;
		end
	end
...
function DialogCell:SetContext(text)
	...
	local bWrap, leftStr = UIUtil.GetWrapLeftStringOfEnglishText (self.contentlabel, newText);
	--todo xde
