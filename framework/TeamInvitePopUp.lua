...
function TeamInvitePopUp:ClickMemberEvent(cellCtl)
  if cellCtl then
    ...
    elseif cellCtl.eventType == "Invite" then
      ...
      elseif not teamProxy:IsInMyTeam(data.id) then
...
function TeamInvitePopUp:HandleLongPress(param)
  ...
  if not GameConfig.SystemForbid.TabNameTip then
    if isPressing then
      ...
      TipManager.Instance:TryShowVerticalTabNameTip(TeamInvitePopUp.TabName[go.name], sp, NGUIUtil.AnchorSide.Down, {140, -58})
    else
