...
function MainViewChatMsgPage:InitShow()
  ...
  self.fadeBtnTweenValue = {
    Vector3(self.fadeBtnTween.from.x, 62.5, self.fadeBtnTween.from.z),
  ...
  self.chatGridTweenValue = {
    Vector3(self.chatGridTween.from.x, 68.5, self.chatGridTween.from.z),
  ...
  self.buttonGridTweenValue = {
    Vector3(self.buttonGridTween.from.x, 146, self.buttonGridTween.from.z),
...
function MainViewChatMsgPage:UpdateChatRoom(note)
  if note then
    ...
    if data and data:GetCellType() ~= ChatTypeEnum.SystemMessage and ChatRoomProxy.Instance:IsPlayerSpeak(data:GetChannel()) then
