  ...
  local press = function(obj, state)
    if state then
      if self.data ~= nil and self.data.staticData ~= nil then
        if self.indexInList ~= nil and ShortCutProxy.Instance:GetUnLockSkillMaxIndex() - self.indexInList < 4 then
...
function ShortCutSkill:UpdatePreCondition()
  if self.data then
    ...
    if instance:HasFitSpecialCost(self.data) then
      self.notFitCheck:RemoveReason(ShortCutSkill.FitSpecialCost)
    else
      self.notFitCheck:SetReason(ShortCutSkill.FitSpecialCost)
    end
    if self.data.staticData ~= nil and instance:IsSubSkillFitPreCondition(self.data) then
...
function ShortCutSkill:CheckEnableUseSkill(val)
  ...
  if self.data and self.data.staticData then
    if Game.Myself.data:GetAttackSkillIDAndLevel() == self.data.staticData.id and SkillProxy.Instance:CanMagicSkillUse(self.data) then
      val = true
    end
