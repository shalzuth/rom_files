...
function MsgManager.ShowMsgByID(id, ...)
  ...
  if data ~= nil then
    ...
    helplog("Table_Sysmsg id" .. id .. "\231\173\150\229\136\146\230\178\161\230\156\137\228\184\138\228\188\160\239\188\129\239\188\129\239\188\129")
  end
end
function MsgManager.ShowMsgByIDTable(id, param, roleid, lockreason)
  local data = Table_Sysmsg[id]
  if data ~= nil then
    local removeTime = SystemMsgRemove[id]
    MsgManager.ShowMsgTable(data.Title, data.Text, data.Type, param, roleid, data, removeTime, lockreason)
  end
end
...
function MsgManager.ShowMsgTable(title, text, type, param, roleid, data, removeTime, lockreason)
  local handler, index
  for i = 1, #MsgManager.BitIndexs do
    index = MsgManager.BitIndexs[i]
    if BitUtil.valid(type, index) then
      if BitUtil.band(type, index) > 0 then
        ...
        if handler ~= nil then
          handler(title, text, param, roleid, data, removeTime, lockreason)
        end
      end
    else
      break
