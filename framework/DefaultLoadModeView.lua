...
DefaultLoadModeView.Mode = {Card = 1, Model = 2}
...
function DefaultLoadModeView:TryLoadPic(bgName)
  ...
  PictureManager.ReFitMobileScreenHeight(self.bg)
end
...
function DefaultLoadModeView:Progress(value)
  if self.mode == DefaultLoadModeView.Mode.Card and self.showCardPercent and value >= self.showCardPercent then
    self:PlayCard()
  end
...
function DefaultLoadModeView:RandomMode(sceneinfo)
  ...
  if GameConfig.BgRandom then
    ...
    if randomBgConfig and #randomBgConfig >= 2 and math.random(0, 100) <= randomBgConfig[2] * 100 then
