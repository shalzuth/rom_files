...
function SealTaskPopUp:UpdateSealTasks()
  ...
  local aceptSealId, aceptSeal, firstSealMap = SealProxy.Instance.nowAcceptSeal, nil, nil
  for i = 1, #list do
    local sealid = list[i]
...
function SealTaskPopUp:UpdateSealRepairTimes()
  ...
  if not (donetimes < maxtimes) or not donetimes then
    donetimes = maxtimes
  end
