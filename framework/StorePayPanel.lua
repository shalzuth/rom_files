...
function StorePayPanel:LoadView()
  ...
  if self.curProduct ~= nil then
    ...
    self.paytitle.text = "NT$ " .. (self.curProduct and FuncZenyShop.FormatMilComma(self.curProduct.Rmb) or "")
    self.zenytitle = self:FindGO("ZenyTitle"):GetComponent(UILabel)
    self.zenytitle.text = self.curProduct and FuncZenyShop.FormatMilComma(self.curProduct.Count) or ""
    ...
    if self.curProduct.MonthLimit ~= nil then
      ...
      local purchaseTimes = UIModelZenyShop.Ins():GetLuckyBagPurchaseTimes(productId)
      if purchaseTimes == 0 then
        if productId == 1777 then
          productId = 1666
        end
        local activity = UIModelZenyShop.Ins():GetProductActivity(productId)
      ...
      if purchaseTimes ~= -1 then
        if purchaseTimes >= self.curProduct.MonthLimit then
          ...
          self.productDesLabel.text = UIListItemCtrlLuckyBag.PaintColorPurchaseTimes(string.format(ZhString.LuckyBag_PurchaseTimesMonth, purchaseTimes, UIListItemCtrlLuckyBag.PaintColorMorePurchaseTimes(self.curProduct.MonthLimit)))
        end
      else
