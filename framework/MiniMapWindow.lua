...
function MiniMapWindow:InitView()
  ...
  self.destTransEffect = self:PlayUIEffect(EffectMap.UI.MapPoint, self.d_symbolParent.gameObject, false, nil, self)
  ...
  self.tipTransEffect = self:PlayUIEffect(EffectMap.UI.MapPoint2, self.d_symbolParent.gameObject, false, nil, self)
  ...
  for key, data in pairs(datas) do
    ...
    if self:ObjIsNil(symbolObj) then
      if type(createFunc) == "function" then
        ...
        symbolObj.name = tostring(key)
      end
    elseif type(updateFunc) == "function" then
  ...
  if not self:ObjIsNil(symbolObj) then
    ...
    if nextSceneID then
      ...
      if info then
        ...
        tempV3:Set(0.85, 0.85, 0.85)
        info.transform.localScale = tempV3
        local label = info:GetComponent(UILabel)
        label.depth = 31
        label.text = string.char(data:GetParama("index") + 64)
      end
    end
  end
  return symbolObj
...
function MiniMapWindow:UpdateExitPoints()
  ...
  self:HelpUpdatePoses(self.exitPointMap, self.exitMapDatas, MiniMapWindow._CreateExitPoint, MiniMapWindow._UpdateExitPoint, MiniMapWindow._RemoveExitPoint)
end
function MiniMapWindow:UpdateNpcPointMap(datas, isRemoveOther)
  self:HelpUpdatePoses(self.npcMap, datas, nil, nil, nil, isRemoveOther)
end
...
function MiniMapWindow:UpdateServerNpcPointMap(datas, isRemoveOther)
  self:HelpUpdatePoses(self.servernpc_map, datas, nil, nil, nil, isRemoveOther)
end
...
function MiniMapWindow:UpdateTeamMemberSymbol(datas, isRemoveOther)
  self:HelpUpdatePoses(self.teamMap, datas, nil, MiniMapWindow._UpdateTeamSymbol, nil, isRemoveOther)
end
...
function MiniMapWindow:UpdateQuestNpcSymbol(datas, isRemoveOther)
  if self:IsActive() then
    self:HelpUpdatePoses(self.questMap, datas, MiniMapWindow._CreateQuestNpcSymbol, MiniMapWindow._UpdateQuestNpcSymbol, MiniMapWindow._RemoveQuestNpcSymbol, isRemoveOther)
  end
end
...
function MiniMapWindow:UpdateQuestFocuses(datas, isRemoveOther)
  self.questFocusDirty = true
  self:HelpUpdatePoses(self.focusMap, datas, MiniMapWindow._CreateQuestFocus, MiniMapWindow._UpdateQuestFocus, MiniMapWindow._RemoveQuestFocus, isRemoveOther)
end
...
function MiniMapWindow:UpdateFocusArrowPos()
  ...
  for key, focus in pairs(self.focusMap) do
    ...
    local arrow = self.focusArrowMap[key]
    if pHalfSize_x < math.abs(dirV3.x) or pHalfSize_y < math.abs(dirV3.y) then
...
function MiniMapWindow:UpdateScenicSpotSymbol(datas, isRemoveOther)
  self:HelpUpdatePoses(self.scenicMap, datas, MiniMapWindow._CreateScenicSpot, MiniMapWindow._UpdateScenicSpot, nil, isRemoveOther)
end
...
function MiniMapWindow:UpdateSealSymbol(datas, isRemoveOther)
  self:HelpUpdatePoses(self.sealMap, datas, MiniMapWindow._CreateSealSymbol, MiniMapWindow._UpdateSealSymbol, nil, isRemoveOther)
end
...
function MiniMapWindow:UpdateMonstersPoses(datas, isRemoveOther)
  self:HelpUpdatePoses(self.monsterMap, datas, MiniMapWindow._CreateMonsterPoints, MiniMapWindow._UpdateMonsterPoints, MiniMapWindow._RemoveMonsterPoints, isRemoveOther)
end
function MiniMapWindow:UpdateTreePoints(datas, isRemoveOther)
  self:HelpUpdatePoses(self.treeMap, datas, nil, nil, nil, isRemoveOther)
end
...
function MiniMapWindow:UpdatePlayerPoses(datas, isRemoveOther)
  self:HelpUpdatePoses(self.playerMap, datas, MiniMapWindow._CreatePlayerPoints, MiniMapWindow._UpdatePlayerPoints, nil, isRemoveOther)
end
...
function MiniMapWindow:UpdateGvgDroiyanInfos(datas, isRemoveOther)
  self:HelpUpdatePoses(self.gvdDoriyanMap, datas, MiniMapWindow._CreateGvgDroiyanInfos, MiniMapWindow._UpdateGvgDroiyanInfos, nil, isRemoveOther)
end
function MiniMapWindow:UpdatePoringFightMapItems(datas, isRemoveOther)
  self:HelpUpdatePoses(self.poringFightDropMap, datas, nil, nil, nil, isRemoveOther)
end
function MiniMapWindow:UpdateTeamPwsInfo(datas, isRemoveOther)
  self:HelpUpdatePoses(self.teamPwsMap, datas, nil, nil, nil, isRemoveOther)
end
