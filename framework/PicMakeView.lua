...
function PicMakeView:ClickCombine(cellctl)
  if cellctl.data then
    ...
    if self.composeData.ROB <= MyselfProxy.Instance:GetROB() then
      ServiceItemProxy.Instance:CallProduce(SceneItem_pb.EPRODUCETYPE_HEAD, self.composeData.id, self.npcinfo.data.id)
...
function PicMakeView:ActiveView(b)
  ...
  if not b or not Color(1, 1, 1, 1) then
  end
  self.blackBg.color = Color(1, 1, 1, 0.00392156862745098)
end
