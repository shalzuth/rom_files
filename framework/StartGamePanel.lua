...
local VIDEO_ID = GameConfig.System.startGameVideoID or 1
local videoName = Table_Video[VIDEO_ID].Name
function StartGamePanel:PlayCG()
  if ApplicationInfo.IsRunOnEditor() then
    return
  end
  if LocalSaveProxy.GetSkipStartGameCG() then
    return
  end
  local iscall = pcall(function(i)
    if ApplicationInfo.IsRunOnWindowns() then
      VideoPanel.PlayVideo(videoName, true)
    else
      LuaUtils.PlayMovie(videoName, true)
    end
  end)
end
...
function StartGamePanel:OnEnter()
  helplog("StartGamePanel:OnEnter")
  OverseaHostHelper:AFRenewTrack()
  FunctionPreload.Me():PreLoadUI()
  self:initData()
  self:InitShow()
...
function StartGamePanel:initView()
  ...
  PictureManager.ReFitMobileScreenHeight(self.bgTexture)
  self:HideConnecting()
  ...
  local agreetmentLabel = self:FindGO("agreetmentLabel")
  agreetmentLabel:SetActive(false)
end
    ...
    if SDKEnable then
      ...
      if isLogined then
        ...
        if Application.platform == RuntimePlatform.WindowsPlayer or Application.platform == RuntimePlatform.WindowsEditor then
          FunctionXDSDK.Ins:Logout(nil, nil)
        else
        end
...
function StartGamePanel:CallServer(acc, deleteRole)
  ...
  if not SDKEnable then
    if ApplicationInfo.GetRunPlatform() == RuntimePlatform.Android and FunctionSDK.Instance.IsInitialized == false then
...
function StartGamePanel:UpdateServerShow()
  if self.serverData then
    ...
    self.serverLab.color = Color(0.615686274509804, 0.615686274509804, 0.615686274509804)
  end
end
...
function StartGamePanel:UpdateServerList(note)
  ...
  if SDKEnable then
    ...
    if serverDatas and #serverDatas > 1 then
      ...
      tmpServer = tmpServer or serverDatas[1]
...
function StartGamePanel:SwitchToSelectScene()
  ...
  if socialShareConfig ~= nil and not BackwardCompatibilityUtil.CompatibilityMode(BackwardCompatibilityUtil.V9) then
