...
function EyeLensesView:AddEvts()
  ...
  function toggleLongPress.pressEvent(obj, state)
    self:PassEvent(TipLongPressEvent.EyeLensesView, {
      state,
      obj.gameObject
    })
  end
  self:AddEventListener(TipLongPressEvent.EyeLensesView, self.HandleLongPress, self)
end
...
function EyeLensesView:ClickReplaceBtn()
  ...
  if queryData.type ~= ShopDressingProxy.DressingType.EYE then
    return
  end
...
function EyeLensesView:RefreshLensesROB(constID, costCount, eyeId)
  ...
  local flag = costCount <= curCount and ShopDressingProxy.Instance:bActived(eyeId, ShopDressingProxy.DressingType.EYE)
...
function EyeLensesView:HandleLongPress(param)
  ...
  if not GameConfig.SystemForbid.TabNameTip then
    if isPressing then
      ...
      TipManager.Instance:TryShowHorizontalTabNameTip(ZhString.EyeLensesView_title, backgroundSp, NGUIUtil.AnchorSide.Left)
    else
