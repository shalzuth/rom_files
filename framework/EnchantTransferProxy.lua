...
function EnchantTransferProxy:GetEnchantInData()
  ...
  for i = 1, #equipEquips do
    ...
    if 0 ~= TableUtility.ArrayFindIndex(self.lotteryStaticId, equip.staticData.id) and _FilterCfg[equip.staticData.Type] and 0 < #equip.enchantInfo:GetEnchantAttrs() then
...
function EnchantTransferProxy:GetEnchantOutData()
  ...
  local equipEquips = BagProxy.Instance.roleEquip:GetItems() or {}
  local bagEquips = BagProxy.Instance:GetBagEquipItems()
  local chooseType = self.chooseData.staticData.Type
  local chooseGuid = self.chooseData.id
  for i = 1, #equipEquips do
    local equip = equipEquips[i]
    if 0 ~= TableUtility.ArrayFindIndex(self.lotteryStaticId, equip.staticData.id) and _FilterCfg[equip.staticData.Type] and equip.equipInfo:CanEnchant() and chooseType == equip.staticData.Type and equip.id ~= chooseGuid then
