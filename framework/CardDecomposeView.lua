...
function CardDecomposeView:FindObjs()
	...
	self.sprite = self:FindGO("Sprite",self.tipLabel.gameObject)
	self.sprite:SetActive(false)
	self.tipLabel.text = ZhString.CardDecomposeViewTitle
	
	local filterRoot =self:FindGO("FilterRoot",self.chooseBor)
	local flabel = self:FindGO("Label",filterRoot):GetComponent(UILabel)
	OverseaHostHelper:FixLabelOverV1(flabel,3,110)
	local cTitle = self:FindGO("Label",self.totalCost.gameObject):GetComponent(UILabel)
	cTitle.pivot = UIWidget.Pivot.Right
	cTitle.transform.localPosition = Vector3(-40,0,0)
	self:FindGO("TitleBg",self.itemRoot):GetComponent(UISprite).width = 340
end
