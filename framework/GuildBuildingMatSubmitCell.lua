...
function GuildBuildingMatSubmitCell:SetData(data, flag)
  ...
  if data then
    ...
    if data.materials and data.materials.id and data.unitCount then
      ...
      if not flag or not string.format(ZhString.GuildBuilding_Submit_Name, Table_Item[data.materials.id].NameZh, ownCount, num) then
      end
      self.name.text = string.format(ZhString.GuildBuilding_Submit_MatNum, ownCount, num)
    end
