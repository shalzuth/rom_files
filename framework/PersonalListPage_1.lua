...
function PersonalListPage:HandleClickItem(cellCtl)
  if cellCtl and cellCtl.data and self.showMode ~= PersonalPicturePanel.ShowMode.ReplaceMode then
    ...
    if cellCtl.status == PersonalPictureCell.PhotoStatus.Success or cellCtl.status == PersonalPictureCell.PhotoStatus.CanReUploading or cellCtl.status == PersonalPictureCell.PhotoStatus.Uploading then
      local viewdata = {
