...
function EquipUpgradePopUp:DoConfirm()
  if #_lackItems > 0 and QuickBuyProxy.Instance:TryOpenView(_lackItems, QuickBuyProxy.QueryType.NoDamage) then
    return
  end
    ...
    for i = 1, #cost do
      ...
      if id ~= 100 then
        if ItemData.CheckIsEquip(id) then
          local equips = BlackSmithProxy.Instance:GetMaterialEquips_ByEquipId(id, cost[i].num, true, nil, upgrade_checkBagTypes)
          for j = 1, #equips do
