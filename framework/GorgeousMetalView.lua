...
function GorgeousMetalView:InitView()
	...
	self.roomtable = self:FindComponent("RoomTable", UITable);
	...
	self.scrollView = self:FindComponent("RoomScrollView", ROUIScrollView);
...
function GorgeousMetalView:JoinRoom(cell)
	...
	-- self:clickGorgeousMetalRoom(cell);
end
...
function GorgeousMetalView:JoinTeam(param)
	...
	if(roomCell and teamCell)then
		...
		if(roomData and teamData)then
			-- self:DoJoinTeam(G_PVP_TYPE, roomData.guid, nil, false, teamData.index);
		end
	end
end
