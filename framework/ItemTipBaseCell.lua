...
ItemTipAttriType = {
  ...
  RecommandReason = 8,
  EquipBaseAttri = 9,
  NextEquipLotteryAttri = 10,
  EquipStrengthRefine = 11,
  EquipSpecial = 12,
  Pvp_EquipBaseAttri = 13,
  Pvp_EquipSpecial = 14,
  EquipEnchant = 15,
  EquipUpInfo = 16,
  EquipUpMaterial = 17,
...
function ItemTipBaseCell:UpdateCountChooseBordButton(showMultipleMsg)
  ...
  self:_helpActiveButton(useMax > self.chooseCount, self.countChoose_AddButton_Sp1, self.countChoose_AddButton_Sp2, self.countChoose_AddButton_Collider)
  self:_helpActiveButton(self.chooseCount > 1, self.countChoose_MinusButton_Sp1, self.countChoose_MinusButton_Sp2, self.countChoose_MinusButton_Collider)
end
...
function ItemTipBaseCell:UpdateAttriContext()
  ...
  if self.data then
    ...
    self:UpdateRecommandReasonInfo(self.data)
...
function ItemTipBaseCell:UpdateNormalItemInfo(data)
  ...
  if typedata and typedata.Name then
    local typeStr = {}
    if Table_Checkrate[sData.id] then
      typeStr.needInfo = true
      typeStr.itemId = sData.id
    end
  ...
  if NOSHOW_GETLIMIT_IDMAP[sData.id] == nil and limitCfg and limitCfg.type ~= nil then
    ...
    if not limitCfg.source or not limitCfg.source[1] then
      local sourceId
    end
    ServiceItemProxy.Instance:CallGetCountItemCmd(itemid, nil, sourceId)
  end
  local needManualShow, inManualStr, unlockManualStr = false, nil, nil
  ...
  if (data:IsMarriageCertificate() or data:IsMarriageRing()) and weddingData then
    ...
    local lintstr = ChangeZoneProxy.Instance:ZoneNumToString(weddingData.zoneid)
    weddingDesc.label[2] = string.format(ZhString.ItemTip_WeddingTip, weddingData.myname, partnername, formattime, lintstr)
    self.contextDatas[ItemTipAttriType.Desc] = weddingDesc
  end
  if sData.NoStorage and 0 < sData.NoStorage and sData.NoStorage ~= 1 then
    local noStroage = {}
    local storgeDesc = ""
    local index = 0
  ...
  if useItemData then
    ...
    if useItemData.UseLimitTimes then
      local usedtimes = data.usedtimes or 0
      local usetime = {}
      usetime.label = string.format(ZhString.ItemTip_UseTime, usedtimes, useItemData.UseLimitTimes)
      usetime.hideline = true
      self.contextDatas[ItemTipAttriType.UseTime] = usetime
    end
  end
  if itemid == 5650 then
...
local EquipUpgrade_EquipLevel_SpriteMap = {
  [1] = {
    Active = "tips_icon_1",
    InActive = "tips_icon_1b"
  },
  [2] = {
    Active = "tips_icon_2",
    InActive = "tips_icon_2b"
  },
  [3] = {
    Active = "tips_icon_3",
    InActive = "tips_icon_3b"
  }
}
...
function ItemTipBaseCell:UpdateEquipAttriInfo(data)
  ...
  if equipInfo then
    ...
    if 0 < equipInfo.strengthlv then
      local tiplabel
      if isMyItem then
    ...
    if 0 < equipInfo.strengthlv2 then
      local additionalAttrs = ItemFun.calcStrengthAttr(data.staticData.Quality, equipInfo.equipData.EquipType, equipInfo.strengthlv2)
      local effects = {}
      ...
      strengthlab2 = string.format(ZhString.ItemTip_StrengthLv2, equipInfo.strengthlv2, PropUtil.FormatEffects(effects, 1, " +"))
    end
    if 0 < equipInfo.refinelv then
        ...
        for i = 1, #combineEffects do
          ...
          if buffData then
            ...
            if combineEffect.isWork then
              enchant.label = enchant.label .. string.format("[c][222222]%s:%s[-][/c]", tostring(buffData.BuffName), tostring(buffData.BuffDesc))
            else
              enchant.label = enchant.label .. string.format("[c][9c9c9c]%s:%s(%s)[-][/c]", tostring(buffData.BuffName), tostring(buffData.BuffDesc), tostring(combineEffect.WorkTip))
            end
          end
        end
    ...
    if upgradeData then
      ...
      if maxUplv > 0 then
        ...
        local upgradeInfo = {}
        upgradeInfo.label = {
          ZhString.ItemTip_UpgradeTip
        }
        local hasUnlocklv
        for i = 1, maxUplv do
          local buffid = equipInfo:GetUpgradeBuffIdByEquipLv(i)
          if buffid then
            local text = Table_Buffer[buffid] and Table_Buffer[buffid].Dsc or buffid .. " No Buff Dsc"
            local spriteText
            if i > equiplv then
        ...
        if equiplv < maxUplv or equiplv == maxUplv and upgradeData.Product then
          ...
          local umtext = {}
          if shortcutID and shortcutID then
...
function ItemTipBaseCell:UpdateFoodInfo(data)
  if data:IsFood() then
    ...
    for k, v in pairs(cooklvAttr) do
      ...
      str = self:ActiveText(str, maxCooklv <= cookerlv)
    ...
    for k, v in pairs(tasterlvAttr) do
      ...
      str = self:ActiveText(str, maxTasterlv <= tasterlv)
      table.insert(foodAdvInfo.label, str)
    end
...
function ItemTipBaseCell:UpdateRecommandReasonInfo(data)
  if nil == data then
    return
  end
  if Game.Myself.data:GetEquipedItemNum(data.staticData.id) > 0 then
    return
  end
  if 0 < BagProxy.Instance:GetAllItemNumByStaticID(data.staticData.id) then
    return
  end
  local recommandDesc = {}
  local recommandReasonData = Table_recommend_reason[data.staticData.id]
