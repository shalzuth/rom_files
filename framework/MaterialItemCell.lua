...
function MaterialItemCell:Init()
  ...
  self.NumLabel.fontSize = 21
  self.NumLabel.pivot = UIWidget.Pivot.Center
end
function MaterialItemCell:SetData(data)
  ...
  if data.neednum then
    ...
    self:UpdateNumLabel(colorStr .. data.num .. "[-][/c]/" .. data.neednum)
  else
  ...
  self.NumLabel.transform.localPosition = Vector3(0, -26, 0)
end
...
function MaterialItemCell:SetDisCountTip(b, pct)
  if self.discountTip then
    if b then
      ...
      convert2OffLbl(self.discountTip_Label)
