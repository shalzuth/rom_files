...
function BlackSmithProxy:GetMaterialEquips_ByVID(itemData, bagTypes, matchCall, matchCallParam, noCheckFavoriteMaterial)
  ...
  if bagTypes == nil then
    ...
    for i = 1, #bagItems do
      ...
      if isComposeEquip then
        valid = valid and Func_CheckEquip_IsComposeMaterial(itemData, item)
      else
        valid = valid and Func_CheckEquip_SameVID(itemData, item)
      end
...
function BlackSmithProxy:GetMyHRefineEffects(pos, typelevel, refinelv)
  ...
  if typelevel ~= nil then
    ...
    resuleEffectMap = self:Get_TotalHRefineEffect_Map(pos, typelevel, level, myclass, refinelv)
  else
    resuleEffectMap = {}
    local maxType = self:GetMaxHRefineTypeOrLevel(pos)
    for k = 1, maxType do
