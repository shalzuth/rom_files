      ...
      for i = 1, #combineEffects do
        ...
        if combineEffect then
          ...
          if temp.isWork then
            ...
            temp.combineTip = string.format("%s:%s(%s)", tostring(combineEffect.buffData.BuffName), tostring(combineEffect.buffData.BuffDesc), tostring(combineEffect.WorkTip))
          end
          table.insert(resultEffect, temp)
        end
      end
...
function EnchantView:ExcuteEnchant()
  if self.nowItemData then
    if not self:HasErrorEnchantInfo(self.nowItemData) and not self.material_enough then
