...
RepositoryView.Tab = {RepositoryTab = 1, CommonTab = 2}
...
function RepositoryView:FindObjs()
  ...
  if not GameConfig.SystemForbid.Store then
    self.commonTab.gameObject:SetActive(true)
  else
    self.commonTab.gameObject:SetActive(false)
  end
end
...
function RepositoryView:InitShow()
  ...
  local tabList = {
    self.repositoryTab.gameObject,
    self.commonTab.gameObject
  }
...
function RepositoryView:HandleLongPress(param)
  ...
  if not GameConfig.SystemForbid.TabNameTip then
    if isPressing then
      ...
      TipManager.Instance:TryShowHorizontalTabNameTip(RepositoryView.TabName[go.name], backgroundSp, NGUIUtil.AnchorSide.Right, {210, 8})
    else
