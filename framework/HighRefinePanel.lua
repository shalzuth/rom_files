...
function HighRefinePanel:UpdateHRefineEquipPosInfo(initChoose)
  local posDatas = {}
  if not BlackSmithProxy.Instance:GetHighRefinePoses() then
    local showPoses = {5}
  end
...
function HighRefinePanel:UpdateChooseHRefineTypeInfo(levelType)
  ...
  if curlv == self.maxHRefineLevel then
    ...
    local effectmap = blackSmithProxy:Get_TotalHRefineEffect_Map(self.equipPos, levelType, curlv + 1, self.myclass)
    if effectmap then
  ...
  local effectmap = blackSmithProxy:Get_TotalHRefineEffect_Map(self.equipPos, levelType, curlv, self.myclass)
  if effectmap then
    local proKey, proValue = next(effectmap)
    if fakeGet then
      proValue = 0
