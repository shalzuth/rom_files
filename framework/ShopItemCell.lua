...
function ShopItemCell:SetData(data)
  ...
  data = _HappyShopProxy:GetShopItemDataByTypeId(id)
  ...
  if data then
    ...
    if data.Discount ~= nil and data.ItemCount ~= nil and data.ItemID ~= nil then
      ...
      if discount < 100 then
        ...
        convert2OffLbl(self.sellDiscount)
        local bg = self:FindGO("Bg1", self:FindGO("sellDiscount")):GetComponent(UISprite)
        bg.spriteName = "shop_icon_off"
      else
