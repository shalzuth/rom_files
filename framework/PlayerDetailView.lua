...
function PlayerDetailView:ClickEquip(cellCtl, isClickMyself)
  if cellCtl ~= nil and self.chooseEquip ~= cellCtl then
    ...
    if data then
      if not isClickMyself or not {190, 0} then
        local offset = {-190, 0}
      end
...
function PlayerDetailView:InitTabEvent()
  ...
  local tabLbl = self:FindGO("Label", self.fashionTab):GetComponent(UILabel)
  tabLbl.text = "\229\164\150\232\167\130"
...
function PlayerDetailView:UpdateRightState()
  self.roleInfo:SetActive(self.rightState == PlayerDetailView.State.Equip or self.rightState == PlayerDetailView.State.Fashion)
  self.roleBord:SetActive(self.rightState == PlayerDetailView.State.Equip or self.rightState == PlayerDetailView.State.Fashion)
  self.roleEquips:SetActive(self.rightState == PlayerDetailView.State.Equip)
  local attriVisible = self.rightState == PlayerDetailView.State.Attri
  self.playerAttriHolder:SetActive(attriVisible)
  if attriVisible then
