...
SetView.IsFPSShow = "SetView_IsFPSShow "
SetView.TabName = {
...
function SetView:FindObj()
  ...
  self:FindDynamicObj()
  OverseaHostHelper:FixLabelOverV1(self.gameTime, 3, 310)
end
...
function SetView:AddButtonEvt()
  ...
  local label = self:FindGO("Label", backLoginBtn):GetComponent(UILabel)
  ...
  for i, v in ipairs(self.toggleList) do
    do
      ...
      function longPress.pressEvent(obj, state)
        self:PassEvent(TipLongPressEvent.SetView, {state, i})
      end
    end
    break
  end
  ...
  local lbl = self:FindGO("Label", SystemSettingToggle):GetComponent(UILabel)
  lbl.width = 25
end
...
function SetView:_CreateDynamicToggle(togObjName, togName, pageObjName)
  ...
  togLabel.gameObject:SetActive(false)
  local togIcon = self:FindGO("Icon", toggleObj):GetComponent(UISprite)
  togIcon.transform.localPosition = Vector3(-6, 15, 0)
  togIcon.gameObject:SetActive(true)
  togIcon.spriteName = "setv4"
...
function SetView:AddDynamicButtonEvt()
  local togObj = self:FindGO("toggles")
  local ServiceSettingToggle = self:FindGO("ServiceSetting", togObj)
  local Background = self:FindGO("Background", ServiceSettingToggle):GetComponent(UISprite)
  Background.depth = -1
