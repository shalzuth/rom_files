...
function ServiceNUserAutoProxy:CallVarUpdate(vars) 
...
function ServiceNUserAutoProxy:CallEffectUserCmd(effecttype, charid, effectpos, pos, effect, msec, times, index, opt, posbind, epbind, delay, id, dir) 
...
function ServiceNUserAutoProxy:CallUpdateShopGotItem(item, discountitem, limititem) 
	...
	if(discountitem ~= nil )then
		if(discountitem.id ~= nil )then
			msg.discountitem.id = discountitem.id
		end
	end
	if(discountitem ~= nil )then
		if(discountitem.count ~= nil )then
			msg.discountitem.count = discountitem.count
		end
	end
	if(limititem ~= nil )then
		if(limititem.id ~= nil )then
			msg.limititem.id = limititem.id
		end
	end
	if(limititem ~= nil )then
		if(limititem.count ~= nil )then
...
function ServiceNUserAutoProxy:CallBattleTimelenUserCmd(timelen, totaltime, musictime, estatus) 
...
function ServiceNUserAutoProxy:CallCountDownTickUserCmd(type, tick, time, sign, extparam) 
...
function ServiceNUserAutoProxy:CallKFCShareUserCmd() 
...
function ServiceNUserAutoProxy:CallServantRewardStatusUserCmd(items) 
...
function ServiceNUserAutoProxy:CallProfessionBuyUserCmd(branch, success) 
