...
MainViewFightPage.TickType = {DesertWolfPvpCountDown = 1, GorgeousMetalPvpCount = 2}
...
function MainViewFightPage:Init()
  ...
  self:initData()
  self:resetData()
  self.name.fontSize = 21
  self.scoreLabel.fontSize = 17
  self.coinCountLable.fontSize = 17
end
...
function MainViewFightPage:initData()
  if not GameConfig.PVPConfig or not GameConfig.PVPConfig[1] then
    local pvpCg
  end
...
function MainViewFightPage:initView()
  ...
  OverseaHostHelper:FixLabelOverV1(self.name, 3, 180)
end
...
function MainViewFightPage:ItemUpdate(note)
  ...
  local num = 0
  num = userdata and (userdata:Get(UDEnum.PVPCOIN) or 0)
...
function MainViewFightPage:SetYoyoData(fightInfo)
  if fightInfo.player_num ~= self.player_num or fightInfo.score ~= self.score then
    ...
    self.scoreLabel.text = string.format(ZhString.MainViewFightPage_YoyoPvpScore, fightInfo.player_num, self.yoyoPvpToalNumPl, score)
  end
  self:setScoreLabelX()
end
...
function MainViewFightPage:SetDesertWolfData(fightInfo)
  if fightInfo.my_teamscore ~= self.my_teamscore or fightInfo.enemy_teamscore ~= self.enemy_teamscore then
    ...
    self.scoreLabel.text = string.format(ZhString.MainViewFightPage_DesertWolfPvpScore, my_teamscoreStr, self.DesertWolfPvpTotalScore, enemy_teamscoreStr, self.DesertWolfPvpTotalScore)
  end
  self:setScoreLabelX()
...
function MainViewFightPage:SetGorgeousMetalData(fightInfo)
  if fightInfo.remain_hp ~= self.remain_hp then
    ...
    self.scoreLabel.text = string.format(ZhString.MainViewFightPage_GorgeousMetalPvpScore, fightInfo.remain_hp)
  end
  self:setScoreLabelX()
  if fightInfo.starttime ~= self.starttime then
    self.starttime = fightInfo.starttime
