...
function UIViewControllerRolesList:Init()
  ...
  self.noticeBord:SetActive(false)
end
function UIViewControllerRolesList:checkShow(option)
  ...
  if self.rightOpt == self.rightMax and self.leftOpt == self.leftMax then
    if PlayerPrefs.GetString("TestForce") ~= "1" then
      UIUtil.PopUpConfirmYesNoView("\230\143\144\231\164\186", "\230\152\175\229\144\166\229\188\186\229\136\182\232\174\190\228\184\186\228\184\173\230\150\135", function()
        PlayerPrefs.SetString("TestForce", "1")
        Game.Me():BackToLogo()
      end, function()
      end, nil, "\230\152\175\231\154\132", "\228\184\141\232\166\129")
    else
      UIUtil.PopUpConfirmYesNoView("\230\143\144\231\164\186", "\229\183\178\231\187\143\229\188\186\229\136\182\228\184\173\230\150\135\239\188\140\230\152\175\229\144\166\232\167\163\233\148\129", function()
...
function UIViewControllerRolesList:OnEnter()
  ...
  for k, v in pairs(rolesInfo.data) do
    ...
    if not (effective_rolecount > 1) then
    end
  end
  ...
  if effective_rolecount <= 1 then
    ...
    self.choosedMainRole = rolesInfo.maincharid ~= nil and 0 < rolesInfo.maincharid
  end
...
function UIViewControllerRolesList:LoadView()
  ...
  if self:IsHaveAnyRole() then
    ...
    if selectedRole > 0 and self.listItemsViewController ~= nil and 0 < #self.listItemsViewController then
...
function UIViewControllerRolesList:SelectNormal(roleID)
  if roleID > 0 and self.listItemsViewController ~= nil and 0 < #self.listItemsViewController then
    for _, v in pairs(self.listItemsViewController) do
      local itemViewController = v
...
function UIViewControllerRolesList:DoSelectNormal(roleID)
  ...
  self.labTitleOfButtoNStartGame.effectColor = Color(0.6196078431372549, 0.33725490196078434, 0, 1)
  ...
  if roleInfo ~= nil then
    ...
    if professionConf ~= nil then
      ...
      self.spBG2OfProfessionIcon.color = ProfessionProxy.Instance:SafeGetColorFromColorUtil(colorKey)
    end
  end
...
function UIViewControllerRolesList:SelectEmpty(index)
  if index > 0 then
    ...
    self.labTitleOfButtoNStartGame.effectColor = Color(0.14901960784313725, 0.24313725490196078, 0.5490196078431373, 1)
...
function UIViewControllerRolesList:DoSetDelete(roleID)
  ...
  self.labTitleOfButtoNStartGame.effectColor = Color(0.4549019607843137, 0.01568627450980392, 0.01568627450980392, 1)
...
function UIViewControllerRolesList:TryLogin()
  if self.choosedMainRole == false then
    if self.effective_rolecount > 0 and self.willCreateRoleSlotIndex ~= nil then
...
function UIViewControllerRolesList:OnClickForButtonLogin()
  ...
  elseif self.willCreateRoleSlotIndex ~= nil and 0 < self.willCreateRoleSlotIndex then
    UIBlackScreen.DoFadeIn(self.spMask, 1, function()
      self:Close()
      LoginRoleSelector.Ins():GoToCreateRole(self.willCreateRoleSlotIndex)
      LoginRoleSelector.Ins():HideSceneAndRoles()
