...
function GorgeousMetalRoomCell:handleClickMember(param)
	self:PassEvent(PvpTeamCellEvent.ClickMember, {self, param[1], param[2]});
end
...
function GorgeousMetalRoomCell:UpdateButtonsActive()
	...
	if(myRoomId == self.data.guid and myRoomType == PvpProxy.Type.GorgeousMetal)then
		if(myRoomState==PvpProxy.RoomStatus.Fighting or 
			myRoomState==PvpProxy.RoomStatus.Success or
			myRoomState==PvpProxy.RoomStatus.End)then
...
function GorgeousMetalRoomCell:ActiveJoinButton(b)
	...
	if(joinButton)then
		...
		if(b)then
			...
			tempColor:Set(170/255,100/255,3/255,1);
