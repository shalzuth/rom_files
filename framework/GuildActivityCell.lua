...
local GUILD_CHALLENGE_ADD = SceneTip_pb.EREDSYS_GUILD_CHALLENGE_ADD or 41;
...
function GuildActivityCell:Init()
	...
	----[[ todo xde ComboEnchance 中的「奖励」强制翻译为 Award
	self:FindGO("Tip", self.multiplySymbol):GetComponent(UILabel).text = "Award"
	--]]
...
function GuildActivityCell:RegistRedTip()
	...
	if(self.data.PanelId and self.data.PanelId == PanelConfig.GuildChallengeTaskPopUp.id)then
		...
		RedTipProxy.Instance:RegisterUI(GUILD_CHALLENGE_ADD, self.transButton, 10, {-7,-7})
...
function GuildActivityCell:UnRegisteRedTip()
	if(self.red_registe)then
		RedTipProxy.Instance:UnRegisterUI(GUILD_CHALLENGE_ADD,self.transButton);
