...
function MVPFightInfoBord:initView()
  ...
  self.deadBossLeftNum = self:FindComponent("deadBossLeftNum", UILabel)
    ...
    for k, v in pairs(bosses) do
      ...
      if npcData and npcData.NameZh ~= "" then
        if v.live and v.live > 0 then
          txt = string.format("\226\151\134%s X %s", npcData.NameZh, v.live)
        else
...
function MVPFightInfoBord:HandleNUserVarUpdate(note)
  if Var_pb.EVARTYPE_MVPREWARDNUM then
    ...
    if not (mvpLeft >= 0) or not mvpLeft then
      mvpLeft = 0
    end
    ...
    if not (miniLeft >= 0) or not miniLeft then
      miniLeft = 0
    end
    self.miniLeftNum.text = string.format(ZhString.MVPFightInfoBord_MiniLeftTime, miniLeft)
    local deadBossLeft = MyselfProxy.Instance:getVarValueByType(Var_pb.EVARTYPE_DEADBOSS_COUNT_MVP) or 0
    deadBossLeft = GameConfig.RaidDeadBoss.mvp_battle.week_deadboss_count - deadBossLeft
    if not (deadBossLeft >= 0) or not deadBossLeft then
      deadBossLeft = 0
    end
    self.deadBossLeftNum.text = string.format(ZhString.MVPFightInfoBord_DeadBossLeftTime, deadBossLeft)
  end
end
