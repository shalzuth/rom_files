...
EquipRefineBord.DefineColor = {
  SelectedButtonColor_Normal = LuaColor(0.7019607843137254, 0.8509803921568627, 0.9450980392156862, 1),
  SelectedTextColor_Normal = LuaColor(0.11764705882352941, 0.3137254901960784, 0.7372549019607844, 1),
  SelectedButtonColor_Safe = LuaColor(1, 0.9098039215686274, 0.5490196078431373, 1),
  SelectedTextColor_Safe = LuaColor(0.7647058823529411, 0.47058823529411764, 0.16470588235294117, 1),
  EffectRefineBtnColor_Normal = LuaColor(0.03529411764705882, 0.10588235294117647, 0.35294117647058826, 1),
  EffectRefineBtnColor_Safe = LuaColor(0.7803921568627451, 0.4235294117647059, 0.12549019607843137, 1),
...
local DEFAULT_MATERIAL_SEARCH_BAGTYPES, REFINE_MATERIAL_SEARCH_BAGTYPES, REPAIR_MATERIAL_SEARCH_BAGTYPES, REPAIR_MATERIAL_CFG
...
function EquipRefineBord:Init()
  ...
  self.tempSprite.transform.localPosition = Vector3(-133, -100, 0)
end
...
function EquipRefineBord:InitBord()
  ...
  self.tempSprite = self:FindGO("Sprite", self.emptyBord_TipS2)
  self.tempLabel = self:FindGO("Label (1)", self.emptyBord_TipS2)
  self.tempSprite:SetActive(false)
  self.tempLabel:GetComponent(UILabel).text = OverSea.LangManager.Instance():GetLangByKey("\232\175\183\231\130\185\229\135\187\226\128\156   \226\128\157\233\128\137\230\139\169\230\137\128\233\156\128\231\178\190\231\130\188\231\154\132\232\163\133\229\164\135"):gsub("   ", "+")
  self.tempLabel:GetComponent(UILabel).width = 500
...
function EquipRefineBord:UpdateRefineModeUI()
  ...
  if refineMode == EquipRefineBord_RefineMode.Normal then
    ...
    self:_helpActiveButton(false, self.safeRefineButton_Sp, self.safeRefineButton_Collider, self.safeRefineButton_Label, ColorUtil.ButtonLabelOrange)
    self:_helpActiveButton(true, self.refineButton_Sp, self.refineButton_Collider, self.refineButton_Label, ColorUtil.ButtonLabelBlue, "com_btn_1")
  elseif refineMode == EquipRefineBord_RefineMode.Safe then
    ...
    self:_helpActiveButton(nowData.equipInfo.refinelv < SAFEREFINE_MAX_LEVEL, self.safeRefineButton_Sp, self.safeRefineButton_Collider, self.safeRefineButton_Label, ColorUtil.ButtonLabelOrange)
    self:_helpActiveButton(false, self.refineButton_Sp, self.refineButton_Collider, self.refineButton_Label, ColorUtil.ButtonLabelBlue, "com_btn_1")
  end
end
...
function EquipRefineBord:DoRefine(nowData)
  ...
  if self.refineMode == EquipRefineBord_RefineMode.Safe then
    ...
    ServiceItemProxy.Instance:CallEquipRefine(nowData.id, composeId, nil, nil, self.refine_npcguid, isRealSafe, selectSafeEquipIds)
  ...
  if isSafeMode then
    ...
    if needNum and needNum > 0 then
      ...
      if #self.safeRefine_selectEquipIds == 0 then
        ...
        local nonFavoriteNum = 0
        for i = 1, #datas do
          table.insert(self.safeRefine_selectEquipIds, datas[i].id)
          nonFavoriteNum = nonFavoriteNum + 1
        end
        if needNum > nonFavoriteNum then
          local lackItem = {
            id = lackEquipId,
            count = needNum - nonFavoriteNum
          }
          table.insert(self.refine_lackMats, lackItem)
        else
          for i = needNum + 1, nonFavoriteNum do
            self.safeRefine_selectEquipIds[i] = nil
          end
        end
      end
...
function EquipRefineBord:UpdateRepairInfo()
  ...
  if effectName and effectAddValue then
    ...
    local pro, isPercent = Game.Config_PropName[effectName], false
    isPercent = pro and pro.IsPercent == 1
    if isPercent then
...
function EquipRefineBord:ShowChooseRefineMaterialBord()
  ...
  local datas = BlackSmithProxy.Instance:GetMaterialEquips_ByVID(nowData, REFINE_MATERIAL_SEARCH_BAGTYPES, nil, nil, true)
  ...
  self.materialchooseBord:Show(nil, self._MaterialChooseFunc, self, self._MaterialChooseValidFunc, self, ZhString.EquipRefineBord_InvalidRepairMatTip)
