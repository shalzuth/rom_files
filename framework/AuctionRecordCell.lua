...
function AuctionRecordCell:FindObjs()
	...
	OverseaHostHelper:FixLabelOverV1(self.info:GetComponent(UILabel),3,836)
end
...
function AuctionRecordCell:SetData(data)
	...
	if data then
		...
		elseif data.type == AuctionRecordState.SignUpSuccess then
			info = string.format(ZhString.Auction_RecordSignUpSuccess, data:GetItemString(), AuctionProxy.Instance:GetMyItemIndex(data.batchid, data.itemid))
