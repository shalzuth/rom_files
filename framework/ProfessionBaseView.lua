...
function ProfessionBaseView:initView( )
	...
	--todo xde hidden label
	self.professionName.gameObject:SetActive(false) 
	self.professionNameEn.gameObject.transform.localPosition = Vector3(-3,10,0)
	...
	-- local unlockDes = self:FindComponent("unlockDes",UILabel)
	-- unlockDes.text = ZhString.Charactor_ProfessionInfoUnlockDes
end
...
function ProfessionBaseView:showRoleModel( agentData )
	...
	--agentData = agentData or tempTable
...
function ProfessionBaseView:ResetData(agentData )
	...
	if(self.currentPfn~=nil)then
		...
		if(self.currentPfn.Explain)then
			for i=1,#GameConfig.ClassInitialAttr do
				...
				if(value > 0)then
					...
					-- todo xde 这里会显示两次所以修复一下，不知道会不会有其他的坑
		...
		self.scrollView:ResetPosition()	-- self.professionSpc.text = prodata.spc
	end
