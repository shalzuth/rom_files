...
function InnerSingleModel:Update()
  if self.delayTimeFlag and Time.unscaledTime >= self.delayTimeFlag then
...
function InnerSingleModel:PlayEmoji(emoji, rotation)
  ...
  if self.follow then
    ...
    Game.RoleFollowManager:RegisterFollow(self.follow.transform, self.model.complete, tempVector3, tempOffset, RoleDefines_EP.Top, nil, nil)
...
function UIMultiModel:PlayRandomEmoji(index, emojiArray, rotation, durationRange)
  local smodel = self:_GetSingleModelByIndex(index)
  if smodel and emojiArray and #emojiArray > 1 then
