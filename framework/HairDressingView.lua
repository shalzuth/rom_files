...
HairDressingView.LongPressEvent = "HairDressingView_LongPress"
...
function HairDressingView:FindObjs()
	...
	self.chargeNum.overflowMethod = 3
	self.chargeNum.width = 144
end
...
function HairDressingView:AddEvts()
	...
	local toggleList = {self.hairCutToggle, self.hairDyeToggle}
	for i,v in ipairs(toggleList) do
		local longPress = v:GetComponent(UILongPress)
		longPress.pressEvent = function (obj, state)
			self:PassEvent(HairDressingView.LongPressEvent, {state, i});
		end
	end
	self:AddEventListener(HairDressingView.LongPressEvent, self.HandleLongPress, self)
