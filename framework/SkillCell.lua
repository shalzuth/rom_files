...
SkillCell.EnableLineColor = Color(126.0/255.0,204.0/255.0,27.0/255.0,1)
SkillCell.DisableLineColor = Color(115.0/255.0,115.0/255.0,115.0/255.0,1)
-- SkillCell.NameGradientBottom = Color(1,197.0/255.0,58.0/255.0,1)
-- SkillCell.NameEffectColor = Color(1,1,1,1)
...
function SkillCell:UpdateName(skillData)
	...
	if(self.skillName~=skillData.NameZh) then
		local name = OverSea.LangManager.Instance():GetLangByKey(skillData.NameZh) -- todo xde 翻译
		local len = StringUtil.getTextLen(name)
		if(len>5) then
--			name = StringUtil.SubString( name,1,4).."..." --todo xde 要了会显示...
		end
		-- if(self.data.learned) then
			self.skillName.text = name
			UIUtil.WrapLabel(self.skillName)
		-- else
		-- 	self.skillName.text = string.format("[c][%s]%s[-][/c]",ColorUtil.GrayColor_16,name)
		-- end
	end
end
...
function SkillCell:ShowPreview(value)
	...
	else self:Hide(self.previewBtn) end
end
...
function SkillCell:ShowUpgrade(value)
	if(value) then 
		...
		self:Hide(self.previewBtn)
	else self:Hide(self.upgradeBtn.gameObject) end
end

function SkillCell:ShowDowngrade(value)
	if(value) then self:Show(self.levelDelBtn.gameObject)
	else self:Hide(self.levelDelBtn.gameObject) end
end
