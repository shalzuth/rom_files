...
function ChangeHeadView:RevertMyselfHead()
  ...
  if myself then
    if myself.data:IsTransformed() then
      ...
      if userData then
        if not userData:Get(UDEnum.HAIR) then
          local hairID
        end
        if not userData:Get(UDEnum.BODY) then
          local bodyID
        end
        if not userData:Get(UDEnum.SEX) then
          local sex
        end
        if not userData:Get(UDEnum.HAIRCOLOR) then
          local haircolor
        end
        ...
        hairID, headID, faceID, mouthID = headIcon:ParseDisplayLogic(hairID, headID, faceID, mouthID, sex)
...
function ChangeHeadView:SetChooseCell(isChoose)
  ...
  for i = 1, #cells do
    ...
    for j = 1, #cell.childrenObjs do
      ...
      if child.data and child.data.id == self.choosePortrait then
