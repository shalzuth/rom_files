...
if not GameConfig.PackageMaterialCheck.guildBuilding then
  local PACKAGE_MATERIAL = {1, 11}
end
...
function GuildBuildingMatSubmitView:UpdataUI()
  ...
  if self.data then
    ...
    if self.data.staticData.Level ~= 0 or not self.data.staticData.Name then
    end
    self.nameLv.text = string.format(ZhString.GuildBuilding_Title, self.data.staticData.Name, self.data.staticData.Level)
...
function GuildBuildingMatSubmitView:OnEnter()
  ...
  if npcTrans and viewPort and rotation and viewPort and rotation then
...
function GuildBuildingMatSubmitView:_getCameraConfig(npcInfo)
  ...
  if #npcfunction == 1 then
    ...
    if funcCfg and funcCfg.NameEn == "BuildingSubmitMaterial" then
      if param == 1 then
    ...
    for i = 1, #npcfunction do
      ...
      if funcCfg then
        ...
        elseif funcCfg.NameEn == "ArtifactMake" then
          ...
          break
        elseif funcCfg.NameEn == "GuildMaterialShop" then
          viewPort = CameraConfig.GuildMaterial_LvUp_ViewPort
          rotation = CameraConfig.GuildMaterial_LvUp_Rotation
          break
        end
      end
    end
