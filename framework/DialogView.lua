...
function DialogView:UpdateViewData()
  ...
  if npcinfo ~= nil then
    ...
    if self.npcCachePos == nil then
      self.npcCachePos = LuaVector3()
      local p = npcinfo:GetPosition()
      self.npcCachePos:Set(p.x, p.y, p.z)
    end
  end
  self.dialogInfo = {}
  self.dialogIndex = 1
...
function DialogView:UpdateDialoglst(dialoglist)
  if #dialoglist > 0 then
    ...
    for i = 1, #dialoglist do
      local dilg, data = dialoglist[i], nil
...
function DialogView:UpdateDefaultDialog()
  if self.npcdata then
    ...
    if defaultDialogId then
      ...
      self.dialogInfo = {
        DialogUtil.GetDialogData(defaultDialogId)
      }
...
function DialogView:HandleNpcMove(note)
  ...
  if id == self.npcguid then
    ...
    if LuaVector3.Distance(self.npcCachePos, target:GetPosition()) > 0.1 then
      self:CloseSelf()
    end
  end
end
