...
function DialogView:ClickDialogEvent()
	...
	-- 如果上一段对话有选项 则必须点击选项才可以进行下一步 否则无法进行
...
-- ViewData参数
-- 1. dialoglist(Dialog表中的ids or string(s){Text=string,ViceText=string}(s)这种情况是为了处理文本文字大小不一样)
-- 2. dialognpcs(对话里涉及到的NPCIDS)
-- 3. tasks 对话的任务配置(支线任务或者对话任务)
-- 4. callback(对话结束的回调) function(callbackData, optionid, isSuccess)
-- 5. camera(摄像机的Z轴角度)
-- 6. wait(对话结束 摄像机重置的等待时间)
-- 7. questdata(任务对话的任务数据)
-- 8. addconfig( 自主添加的功能配置(npcfunc表中的功能类型) )
-- 9. addleft( 是否添加离开按钮 )
-- 10. addfunc( 自主添加的功能(NameZh = string, event, eventParam, closeDialog, waitClose) )
-- 11. npcinfo( 对话的NPC )
-- 12. subViewId( 对话面板id );
-- 13. midShowFunc(gameObject, showEventParam)( 对话界面中间需要创建其他预设时调用 );
-- 14. midShowFuncParam( 对话界面创建时调用 );
-- 15. forceClickMenu (点击界面不会关闭对话)
