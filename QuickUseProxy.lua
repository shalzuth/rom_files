...
--canClose=true/false????????????????????????,iconStr="xxxx"?????????????????????,btnStr="YYY"????????????,iconType = "itemIcon/npcIcon/skillIcon"??????????????????
--iconID ="item_100"?????????spritename,ClickCall????????????
...
function QuickUseProxy:RemoveNeverEquipedFashion(item,removeAll)
	if(item) then
		-- print(string.format("??????%s..??????:%s",item,tostring(removeAll)))
