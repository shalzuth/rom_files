...
function SkillTip:UpdateUseTimes()
	...
	if(deltaTime>0 and self.data.leftTimes< self.data.maxTimes) then
		self.useCount.text = string.format(ZhString.SkillTip_LeftValuesRefresh,self.data.leftTimes,self.data.maxTimes,DateUtil.ParseHHMMSSBySeconds(deltaTime).."???")
	else
...
--skillData:??????????????????Table_Skill ,orignParam:??????????????????????????? ,fixFunc:????????????????????????????????????????????????RoleAgent????????????????????????
--originZhString ???????????????????????? , fixedZhString???????????????????????????,floatX??????????????????1?????????
