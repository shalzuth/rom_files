	...
	for i=1,#buffProps do
		...
		if(buffData.value > 0)then
			if(buffData.propVO.isPercent)then
				buffDesc = buffDesc .. OverSea.LangManager.Instance():GetLangByKey(buffData.propVO.displayName).."+"..tostring(buffData.value/10) .. "%"; --todo xde ??????buff????????????
			else
				buffDesc = buffDesc .. OverSea.LangManager.Instance():GetLangByKey(buffData.propVO.displayName).."+"..tostring(buffData.value);--todo xde ??????buff????????????
			end
		else
			if(buffData.propVO.isPercent)then
				buffDesc = buffDesc .. OverSea.LangManager.Instance():GetLangByKey(buffData.propVO.displayName)..tostring(buffData.value/10) .. "%";--todo xde ??????buff????????????
