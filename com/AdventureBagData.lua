...
function AdventureBagData:getTabByItemAndType(item, type)
  ...
  local tab
  if type == SceneManual_pb.EMANUALTYPE_NPC or type == SceneManual_pb.EMANUALTYPE_MONSTER then
    local configData = GameConfig.AdventureCategoryMapType[item.staticData.ManualMap and Table_Map[item.staticData.ManualMap].Range or "default"]
...
function AdventureBagData:GetItemsByMapID(tabType, mapID)
  if tabType ~= nil then
    local tab = self.tabs[tabType]
    if tab ~= nil then
      return tab:GetItemsByMapID(mapID)
    end
  end
  return self.wholeTab:GetItemsByMapID(mapID)
end
