...
function MyselfTeamData:GetMemberListWithAdd()
	...
	if(#self.memberListWithAdd < 4)then
	...
	for i=1,#list do
		if(list[i].id == Game.Myself.data.id)then
			...
			if(exceptCat ~= true)then
				...
				if(list[i].cat == nil or list[i].cat == 0)then
					table.insert(self.playerMemberList, list[i]);
				end
			end
		end
...
function MyselfTeamData:GetApplyList()
	...
	if(memData and 
		(memData.job == SessionTeam_pb.ETEAMJOB_LEADER or  memData.job == SessionTeam_pb.ETEAMJOB_TEMPLEADER) )then
