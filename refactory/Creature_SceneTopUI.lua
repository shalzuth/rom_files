...
function Creature_SceneTopUI:SetTopFuncFrame(text, icon, clickFunc, clickArgs, creature, sceneUIType)
  ...
  Game.CreatureUIManager:AsyncCreateUIAsset(self.creatureId, SceneTopFuncWord.ResID, follow, Creature_SceneTopUI._CreateSceneTopFuncWord, args, Creature_SceneTopUI._CreateSceneTopFunc_ArgsDeleter)
end
...
function Creature_SceneTopUI:createOrGetTopSingUI()
  ...
  if creature then
    ...
    if not sceneUI or not sceneUI:MaskByType(MaskPlayerUIType.ChatSkillWord) then
      local maskSingIndex
    end
