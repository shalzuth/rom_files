...
function NCreatureWithPropUserdata:InitBuffs(serverData, needhit)
  ...
  if buffDatas and #buffDatas > 0 then
    for i = 1, #buffDatas do
      self:AddBuff(buffDatas[i].id, true, needhit, buffDatas[i].fromid, buffDatas[i].layer, buffDatas[i].level, buffDatas[i].active)
    end
  end
end
...
function NCreatureWithPropUserdata:AddBuff(buffID, init, needhit, fromID, layer, level, active)
  if nil == buffID then
    return
  end
  ...
  if buff == nil then
    ...
    if nil ~= buffStateID and buffStateID > 0 then
...
function NCreatureWithPropUserdata:TryHandleAddSpecialBuff(buffInfo, fromID, layer)
  if buffInfo then
    ...
    if buffeffect.type == BuffType.RideWolf then
      self:Logic_ForceRideAction(1)
    end
  end
end
