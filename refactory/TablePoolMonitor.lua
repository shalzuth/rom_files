...
function TablePoolMonitor:Add(obj)
  ...
  objInfo[1] = obj
  local debugInfo = self:GetTracebackInfo(6)
  local info = {
    [1] = objInfo,
    [2] = debugInfo
  }
  ...
  for i = 1, #self.infos do
    ...
    if obj == info[1][1] then
      if self.checkRemove then
        info[3] = self:GetTracebackInfo(6)
...
function TablePoolMonitor:CreateAutoPool()
  return {
    ...
    PrintSelf = function(v, tag)
      ...
      if type(key) ~= "table" or not key.__cname then
      end
      local str = string.format("%s,%s,%s,%s\n", tostring(key), v.AddTimes, v.MaxPoolSize, v.PoolSize)
      return str
    end
  }
end
