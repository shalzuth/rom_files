...
function SceneSeatManager:MyselfManualGetOffSeat()
  ...
  ServiceNUserProxy.Instance:CallCheckSeatUserCmd(seat:GetID(), false)
  return true
end
...
function SceneSeatManager:_OnSeatAccessed(seatID)
  ...
  ServiceNUserProxy.Instance:CallCheckSeatUserCmd(seatID, true)
...
function SceneSeatManager:UpdateCheckRange()
  ...
  for i = 1, #self.checkRange do
    ...
    if seat ~= nil then
      ...
      if seatPot ~= nil and VectorDistanceXZ(myPos, seatPot) < seat.staticData.FindRange then
        isInTrigger = true
        break
      end
    end
  ...
  if self.checkRangeInTrigger ~= isInTrigger then
    GameFacade.Instance:sendNotification(SceneSeatEvent.TriggerChange, isInTrigger)
...
function SceneSeatManager:Update(time, deltaTime)
  ...
  if self.displaying and 0 < self.displayEndTime and time > self.displayEndTime then
    self:SetDisplay(false)
  end
