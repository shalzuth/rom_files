  ...
  if Game.AutoBattleManager.on and self.on then
    ...
    if not self.myself:IsFakeDead() then
      ...
      if nil ~= props then
        ...
        if (HP <= Hp_Min / 100.0 * maxHP or SP <= Sp_Min / 100.0 * maxSP) and self.skillID ~= nil and self.skillID > 0 and SkillProxy.Instance:SkillCanBeUsedByID(self.skillID) then
          return true
        end
      end
    else
