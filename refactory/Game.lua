...
function Game:ctor(param)
	...
	-- todo xde 初始化game后的修改(原来放在GameLauncher，因为游戏可以切换角色，移到这里)
	self:XDELogin()
...
-- 部分拼接文本（例如：xxx的专属女仆）临时翻译
function simpleReplace(origin)
	local ret = origin
	candidates = {"的专属执事", "的专属女仆", "找不到目标地图", "_的队伍"}
...
-- 将 widget 放到 transform 的右边，偏移 offset
...
function Game:XDELogin()
	...
	if debugResource ~= nil then
		Debug.Log("<color=lime>debugRoot 存在</color>")
		-- enableLog = true
	else
		Debug.Log("<color=lime>debugRoot 不存在</color>")
	end
...
--	track(Table_Dialog, {"Text"})
	...
	redlog(string.format("翻译配置表花费时间  : %.4f", endtime - starttime));


	-- todo xde start 翻译 GameConfig
