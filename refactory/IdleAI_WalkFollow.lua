...
function IdleAI_WalkFollow:Update(idleElapsed, time, deltaTime, creature)
  ...
  if nil ~= self.prevTargetPosition then
    local targetChangedDistance = VectorUtility.DistanceXZ(self.prevTargetPosition, targetPosition)
  ...
  if Phase_Follow == self.phase then
    ...
    if nil == prevTargetPosition or VectorUtility.DistanceXZ(prevTargetPosition, targetPosition) > InnerRange then
