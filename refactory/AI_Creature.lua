...
local InterruptCommands = function(AIClass, cmds, currentCmd, time, deltaTime, creature)
  ...
  if nil ~= interrupts then
    for i = 1, #interrupts do
      ...
      if interruptAIClass.InQueue then
        if nil ~= currentCmd and currentCmd.AIClass == interruptAIClass then
...
local TryInterruptCommands = function(AIClass, cmds, currentCmd, time, deltaTime, creature)
  local interruptCount = 0
  local interrupts = AIClass.Interrupts
  if nil ~= interrupts then
    if nil ~= currentCmd then
      for i = 1, #interrupts do
        local interruptAIClass = interrupts[i]
        if interruptAIClass.InQueue and currentCmd.AIClass == interruptAIClass then
          currentCmd:End(time, deltaTime, creature)
          currentCmd:Destroy()
          currentCmd = nil
          interruptCount = interruptCount + 1
...
local WeakInterruptCommands = function(AIClass, cmds, currentCmd, time, deltaTime, creature)
  local interruptCount = 0
  ...
  if nil ~= interrupts then
    for i = 1, #interrupts do
      local interruptAIClass = interrupts[i]
      if interruptAIClass.InQueue then
        if nil ~= currentCmd and currentCmd.AIClass == interruptAIClass and 1 < currentCmd.interruptLevel then
          currentCmd:End(time, deltaTime, creature)
          currentCmd:Destroy()
          currentCmd = nil
          interruptCount = interruptCount + 1
...
local TryWeakInterruptCommands = function(AIClass, cmds, currentCmd, time, deltaTime, creature)
  local interruptCount = 0
  local interrupts = AIClass.WeakInterrupts
  if nil ~= interrupts then
...
function AI_Creature:_TryExecuteCommands(time, deltaTime, creature)
  ...
  if nil ~= currentCmd then
    ...
    if currentCmd.running or currentCmd.keepAlive then
      ...
      runningCount = runningCount - InterruptCommands(currentCmd.AIClass, runningCmds, nil, time, deltaTime, creature)
      runningCount = runningCount - WeakInterruptCommands(currentCmd.AIClass, runningCmds, nil, time, deltaTime, creature)
    else
      currentCmd:Destroy()
      currentCmd = nil
      self.currentCmd = nil
  ...
  for AIClass, cmd in pairs(requestCmds) do
    runningCount = runningCount - InterruptCommands(AIClass, runningCmds, currentCmd, time, deltaTime, creature)
    runningCount = runningCount - WeakInterruptCommands(AIClass, runningCmds, currentCmd, time, deltaTime, creature)
  end
  if nil ~= currentCmd and not currentCmd:Alive() then
    currentCmd = nil
    self.currentCmd = nil
  end
  if nil ~= currentCmd and #cmdQueue > 0 then
    local nextCmd = cmdQueue[1]
    runningCount = runningCount - TryInterruptCommands(nextCmd.AIClass, runningCmds, currentCmd, time, deltaTime, creature)
    if not currentCmd:Alive() then
      currentCmd = nil
      self.currentCmd = nil
    end
    runningCount = runningCount - TryWeakInterruptCommands(nextCmd.AIClass, runningCmds, currentCmd, time, deltaTime, creature)
    if nil ~= currentCmd and not currentCmd:Alive() then
      currentCmd = nil
      self.currentCmd = nil
    end
  ...
  if nil == currentCmd then
    while #cmdQueue > 0 do
      ...
      if AllowStart(cmd.AIClass, cmd, self, creature) then
  ...
  if not fastForward and #cmdQueue > 0 then
    fastForward = true
  end
  if fastForward then
...
function AI_Creature:BeHolded(masterGUID, creature)
  if nil ~= creature.data then
    self.idleAI_BeHolded:Request_Set(masterGUID, creature.data:GetHoldScale(), creature.data:GetHoldDir(), creature.data:GetHoldOffset())
  else
