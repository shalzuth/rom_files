...
function MapManager:SetCurrentMap(serverData, force)
  ...
  elseif MapManager.Mode.Raid == mode then
    LogUtility.InfoFormat("<color=yellow>Switch Map Raid: </color>{0}, {1}", mapID, raidID)
  ...
  if creatureMaskRange > 0 then
    self.mapInfo[5] = Game.CullingObjectManager:DistanceToLevel(creatureMaskRange)
  else
    self.mapInfo[5] = 99999
  end
  self.mapInfo[6] = raidID
  LogUtility.InfoFormat("<color=yellow>Creature Mask Range: </color>{0}, {1}", self.mapInfo[5], creatureMaskRange)
