...
function PlotStoryProcess:summon( params )
	...
	if(not npcRole)then
		...
		if(npcRole ~= nil)then
			npcRole:Server_SetPosXYZCmd( tempPos[1], tempPos[2], tempPos[3] );
...
function PlotStoryProcess:play_effect( params )
	...
	if(#targets > 0)then
		...
		-- Asset_Effect.PlayOneShotOn( path, container.transform, callback, callArgs )
	end
...
function PlotStoryProcess:wait_pos( params )
	...
	if(target)then
		...
		if(pos)then
			...
			if(targetPos)then
				return LuaVector3.Distance(targetPos, pos) <= distance;
			end
		else
			self:ErrorLog_Plot();
		end
