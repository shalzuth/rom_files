...
function NCreature:PlayDamage_Effect(damage, damageType)
  ...
  if chesetTrans then
    ...
    if damage > 0 then
      ...
      SkillLogic_Base.ShowDamage_Single(damageType, damage, damagePos, HurtNumType.DamageNum_R, color, self)
    elseif damage < 0 then
      if CommonFun.DamageType.Normal_Sp == damageType or CommonFun.DamageType.Treatment_Sp == damageType then
        SkillLogic_Base.ShowDamage_Single(CommonFun.DamageType.Treatment_Sp, -damage, damagePos, nil, nil, self)
      else
        SkillLogic_Base.ShowDamage_Single(CommonFun.DamageType.Treatment, -damage, damagePos, nil, nil, self)
      end
    end
  end
end
