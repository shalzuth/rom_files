...
function TabNameTip:TryFadeOut()
	...
	-- self.delayedFadeOutTween?????????????????????idleDuration???????????????????????????FadeOut
...
function TabNameTip:SetTweenPosition()
	...
	if self.currentState == TNTStateEnum.FADE_IN then
		...
		self.tweenPosition.from = TabNameTip.FadeInPositionOffset[self.fadeInDirection] -- ??????fadeInDirection??????fromPosition 
	elseif self.currentState == TNTStateEnum.FADE_OUT then
		self.tweenPosition.duration = self.fadeOutDuration
		self.tweenPosition.to = TabNameTip.FadeOutPositionOffset[self.fadeOutDirection] -- ??????fadeOutDirection??????toPosition
	end
end
