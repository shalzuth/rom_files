...
function SceneCarrierProxy:CarrierStart(data)
	local carrier = self:Find(data.masterid)
	-- print(carrier.masterID,carrier.line,"server ??????")
	if(carrier) then
...
function SceneCarrierProxy:LeaveCarrier(data)
	local carrier = self:Find(data.masterid)
	-- print(string.format("%s???????????????%s????????????????????????%s ,????????????:%s %s %s,????????????:%s",data.masterid,data.charid,data.newmasterid,data.pos.x,data.pos.y,data.pos.z,tostring(data.all)))
	if(carrier) then
...
function SceneCarrierProxy:Add(data)
	...
	if(data.guid ~= Game.Myself.data.id) then
		if(carrierInfo and carrierInfo.masterid>0 and carrierInfo.id>0) then
			print(data.guid,"add",carrierInfo.id,carrierInfo.masterid,"??????:",carrierInfo.progress)
...
function SceneCarrierProxy:RemoveCarrier(carrier,pos)
	if(carrier) then
		...
		-- print("SceneCarrierProxy????????????",carrier.masterID)
	end
end
