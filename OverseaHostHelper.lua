...
function OverseaHostHelper:FilterLangStr(origin)
    ...
    -- ???????????????????????????
    if(AppBundleConfig.GetSDKLang() == 'id')  and origin:find("???????????????????????????????????????????????????") == nil then
        origin = string.gsub(origin, "???", "+")
...
function OverseaHostHelper:SpecialProcess(str)
    ...
    if(AppBundleConfig.GetSDKLang() == 'vi') then
        ...
        str = str:gsub("Ph??? c??ng c?? 30%% x??c su???t di???n t???u Ca kh??c Noel:Atk%+3%%,Equip.ASPD%+3%%,li??n t???c 10s", "Ph??? c??ng c?? 30%% x??c su???t   di???n t???u Ca kh??c Noel: Atk+3%%, Equip.ASPD+3%%, li??n t???c 10s")
        -- xdlog(str)
    end
