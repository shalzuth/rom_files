...
local OnEffectCreated = function(effectHandle, self)
...
function Buff:SetEffectVisible(visible, deActiveOpt)
...
function Buff:_PlayOneShotEffectOn(creature, effectPath, sePath)
  ...
  return creature.assetRole:PlayEffectOneShotOn(effectPath, self.staticData.EP)
end
function Buff:_PlayOneShotEffectAt(creature, effectPath)
  if nil == effectPath then
    return nil
  end
  if false == self.visible then
  ...
  return creature.assetRole:PlayEffectOneShotAt(effectPath, self.staticData.EP)
end
function Buff:_PlayStartEffect(creature)
  ...
  self:_PlayOneShotEffectAt(creature, GetEffectPath(self.config.Effect_startAt))
  self:CreateWeakData()
  ...
  local effect = self:_PlayOneShotEffectOn(creature, GetEffectPath(self.config.Effect_start), self.staticData.SE_start)
  if nil == effect then
    return false
  end
...
function Buff:_PlayHitEffect(creature)
  ...
  self:_PlayOneShotEffectOn(creature, GetEffectPath(self.config.Effect_hit), self.staticData.SE_hit)
end
function Buff:_PlayEffect(creature)
  ...
  self.effect = creature.assetRole:PlayEffectOn(effectPath, self.staticData.EP, nil, OnEffectCreated, self)
  if nil ~= self.effect then
...
function Buff:_PlayEndEffect(creature)
  ...
  self:_PlayOneShotEffectOn(creature, GetEffectPath(self.config.Effect_end), self.staticData.SE_end)
end
...
function Buff:SetActive(active)
  self:SetEffectVisible(active, BuffOpt)
end
