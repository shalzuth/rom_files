...
function MissionCommandMove:DoLaunch()
  local args = self.args
  if not self.teleport:Launch(args.targetMapID, args.targetBPID, args.targetPos, args.showClickGround, args.allowExitPoint) then
  ...
  if nil ~= args.distance then
    ...
    if currentMapID == args.targetMapID then
      ...
      if self.teleport.running then
        local distance = VectorUtility.DistanceXZ(Game.Myself:GetPosition(), args.targetPos)
