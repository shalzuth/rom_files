...
function FunctionPlayerTip.TalkAuthorization(ptdata)
    ...
    if guildMemberData:IsRealtimevoice() then
    	helplog("不给麦克风 ServiceGuildCmdProxy.Instance:CallOpenRealtimeVoiceGuildCmd(ptdata.id,false)")
    	ServiceGuildCmdProxy.Instance:CallOpenRealtimeVoiceGuildCmd(ptdata.id,false)
    else
    	helplog("给麦克风 ServiceGuildCmdProxy.Instance:CallOpenRealtimeVoiceGuildCmd(ptdata.id,true)")
