...
-- (1:显示在主背包, 2:显示在身上的背包 4:显示在个人仓库, 8:显示在通用仓库, 16:显示在临时背包, 32:显示在手推车背包) 
...
function FunctionItemFunc.GetQuestByItem(data)
	...
	-- helplog("GetQuestByItem data.staticData.id : "..data.staticData.id);
...
function FunctionItemFunc.bGetTaskItemType(staticData)
	if(not Table_UseItem[staticData.id] or not Table_UseItem[staticData.id].UseEffect)then return false;end
...
-- ItemFunc Impletemented begin
function FunctionItemFunc.MakePicEvt(data)
	GameFacade.Instance:sendNotification(UIEvent.ShowUI, {viewname = "PicTipPopUp", data = data});
end
...
function FunctionItemFunc.EquipEvt(data)
	-- 继承规则(已穿戴的装备有强化等级，准备替换的装备无强化等级，触发强化继承)
	...
	if(data)then
		if RepositoryViewProxy.Instance:GetViewTab() == RepositoryView.Tab.CommonTab then
			...
			if itemdata ~= nil then
				...
				if itemdata.petEggInfo ~= nil and itemdata.petEggInfo.lv > roleLevel then
		...
		if(data.cardInfo)then
			...
			if(#filterDatas>0)then
				...
				GameFacade.Instance:sendNotification(UIEvent.JumpPanel, 
					{view = PanelConfig.UseCardPopUp, viewdata = {carddata = data, equipdatas = filterDatas}});
			else
				--没有找到符合要求的装备
			...
			if(cardSlotNum>0)then
				...
				if(#filterCards>0)then
					GameFacade.Instance:sendNotification(UIEvent.ShowUI, {viewname = "SetCardPopUp", viewdata = {itemdata = data, carddatas = filterCards}});
				else
					--没有找到符合要求的卡片
	...
	if(data and (data.equipInfo or data:IsMount()))then
		...
		if(poses)then
			...
			if(not posIsRight)then
				-- 如果存在多个位置 优先寻找空位装备 否则替换评分低的装备
				local nullPos, lowPos, lowEquipScore = nil, nil, nil;
		...
		local transfer = data.equipInfo:CanStrength() and data.equipInfo.strengthlv<=0;
		if(transfer and type(site)=="number")then
			local roleEquipData = BagProxy.Instance.roleEquip:GetEquipBySite(site);
			transfer = roleEquipData and roleEquipData.equipInfo.strengthlv>0;
		end
		if(transfer)then
...
function FunctionItemFunc.TryUseItem(data, target, count)
	...
	-- 领域的鬼判断 loving space
	if sdata.id == 500503 or sdata.id == 500516 then
	...
	-- 使用方式
	if(sdata.UseMode ~= nil and sdata.Type ~=75)then -- 特殊物品使用规则，暴力了
		
		-- 当前职业若是一转，点击阿萨神碑使用会弹出飘字，msgid=25438
	...
	if(useItem)then
		...
		elseif(useEffect.type == "redeemCode")then
			FunctionSecurity.Me():RedeemCode(function ()
				GameFacade.Instance:sendNotification(UIEvent.JumpPanel, {view = PanelConfig.RedeemCodeView,viewdata = {id=sdata.id}})
			end)
			return;
...
function FunctionItemFunc.PickUpFromTempBag(data)
	if(data)then
		if(BagProxy.Instance:CheckItemCanPutIn(nil, data.staticData.id, nil, true))then
...
function FunctionItemFunc.PutInBarrow(data, count)
	-- if(BagProxy.Instance:CheckBagIsFull(BagProxy.BagType.Barrow))then
	-- 	MsgManager.ShowMsgByIDTable(3104);
	-- 	return;
	-- end
...
function FunctionItemFunc.Adventure(data)
	-- if( AdventureDataProxy.Instance:IsCollectionGained(data.staticData.id) )then
	-- 	MsgManager.ShowMsgByIDTable(571);
	-- else
	-- 	MsgManager.ShowMsgByIDTable(572);
	-- end
...
function FunctionItemFunc.PutFood_Self(data, count)
	...
	if overrideNotice and currentEatFoodCount + count > foodMaxCount and itemId ~= 551019 then
		GameFacade.Instance:sendNotification(UIEvent.ShowUI, {viewname = "FoodOverridePopView", foodItemId = itemId, foodGuid = data.id, foodCount = count})
	else
...
function FunctionItemFunc.Hatch(data)
	...
	if(eggInfo ~= nil and eggInfo.name ~= "")then
		...
		GameFacade.Instance:sendNotification(UIEvent.JumpPanel, {view = PanelConfig.PetMakeNamePopUp, viewdata = {etype = 1, item = data}})
	end
end
...
function FunctionItemFunc.Open_Letter(data)
	if data:IsLoveLetter() then
		...
		if panel ~= nil then
			GameFacade.Instance:sendNotification(UIEvent.JumpPanel, {view = panel, viewdata = data.loveLetter})
		end
	end
end

function FunctionItemFunc.Open_MarriageManual(data)
	GameFacade.Instance:sendNotification(UIEvent.JumpPanel, {view = PanelConfig.WeddingManualMainView, viewdata = data})
end

function FunctionItemFunc.Open_MarriageCertificate(data)
	GameFacade.Instance:sendNotification(UIEvent.JumpPanel, {view = PanelConfig.MarriageCertificate, viewdata = data})
end
...
function FunctionItemFunc.Send_WeddingDress(data)
	-- 送出婚纱
	GameFacade.Instance:sendNotification(UIEvent.JumpPanel, {view = PanelConfig.WeddingDressSendView, viewdata = data.id});
end
...
function FunctionItemFunc.OpenRegistTicket(data)
	...
	local startTs = os.time({day=day,month=month,year=year,hour=hour,min=min,sec=sec})
	year,month,day,hour,min,sec = endTime:match(p)
	local endTs = os.time({day=day,month=month,year=year,hour=hour,min=min,sec=sec})
	...
	if(server<startTs or server>endTs)then
		MsgManager.ShowMsgByIDTable(25316);
		return
	end

	if(codeData and codeData.code and codeData.code ~= "")then
		-- helplog("OpenRegistTicket1",codeData.code)
		local url = string.format(ZhString.KFCShareURL,Game.Myself.data.id,codeData.code)
		helplog("OpenURL:",tostring(url))
		Application.OpenURL(url)
	else
		-- helplog("OpenRegistTicket2",data.id)
	...
	if(#rv_Items > 0)then
		local confirmHandler = function ()
			...
			for i=1,#rv_Items do
				...
				local hasstrength2 = equipInfo.strengthlv2 > 0;
...
-- ItemFunc Impletemented end




-- ItemFunc Check Begin
...
function FunctionItemFunc.CheckCombine(itemdata)
	...
	if(combineid)then
		if(combineid~=nil and  Table_Compose[combineid])then
			return ItemFuncState.Active;
		end
	end
	return ItemFuncState.InActive;
