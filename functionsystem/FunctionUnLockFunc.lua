...
function FunctionUnLockFunc:UnlockuSpecialMenuId(menuid)
  ...
  if Func_FindKey(SpeicalMenuIds.Astrolabe, menuid) then
    ...
    return true
  elseif Func_FindKey(SpeicalMenuIds.EvaluateLink, menuid) then
    if self:UnLockMenu(menuid) then
      self:_TryDoEvaluateLink()
    end
    return true
  end
  return false
end
function FunctionUnLockFunc:ShowReview()
  xdlog("\230\152\190\231\164\186\232\175\132\228\187\183")
...
function FunctionUnLockFunc:_TryDoEvaluateLink()
  self:ShowReview()
  OverseaHostHelper:AFTrack("1\230\172\161\232\187\162\232\129\183\229\174\140\228\186\134")
  do return end
  if ApplicationInfo.IsRunOnEditor() then
    MsgManager.ConfirmMsgByID(203, function()
      MsgManager.FloatMsgTableParam(nil, "Error Plat!!")
    end, nil, nil)
    return
  end
...
function FunctionUnLockFunc:UnLockMenu(menuid)
  if not self:RemoveReason(menuid, LockFuncReason.MenuLock) then
    GameFacade.Instance:sendNotification(UIMenuEvent.UnlockMenu)
    return true
  end
  return false
end
