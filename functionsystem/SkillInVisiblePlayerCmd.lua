...
function SkillInVisiblePlayerCmd:HidePlayer(player)
  if player then
    ...
    if TeamProxy.Instance:IsInMyTeam(player.data.id) or player.data.ownerID == Game.Myself.data.id or isPet and TeamProxy.Instance:IsInMyTeam(player.data.ownerID) then
