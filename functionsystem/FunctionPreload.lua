...
function FunctionPreload:InitFrequency_UI()
  ...
  helplog("InitFrequency_UI Frequency_UI", Frequency_UI)
  helplog("InitFrequency_UI Cost:", os.clock() - t1)
end
...
function FunctionPreload:SaveLoadFrequency_UI()
  ...
  wfile:close()
  helplog("SaveLoadFrequency_UI Cost:", filePath, os.clock() - t1)
end
...
function FunctionPreload:PreLoadUI()
  ...
  local t1 = os.clock()
  local _PreLoadManager = PreLoadManager.Ins
  if _PreLoadManager == nil then
    redlog("not find _PreLoadManager")
  end
  local viewPathHelp = ResourcePathHelper.UIView
  for viewname, v in pairs(PreloadUI_Static) do
    _PreLoadManager:PreLoadDefaultResource(viewPathHelp(viewname))
  end
  if Frequency_UI then
    ...
    for i = 1, limitCount do
      _PreLoadManager:PreLoadDefaultResource(viewPathHelp(dynamicConfig[i]))
    end
  end
  helplog("PreLoadUI Cost:", os.clock() - t1)
end
