...
function FunctionPerformanceSetting:Load()
  local isCall = pcall(function(i)
    ...
    if localSet then
      ...
      self.setting.skipStartGameCG = LocalSaveProxy.GetSkipStartGameCG()
    end
  end)
  self:Apply()
end
...
function FunctionPerformanceSetting:SetSkipStartGameCG(value)
  self.setting.skipStartGameCG = value
end
...
function FunctionPerformanceSetting:SetEnd()
  ...
  if self.oldSetting.skipStartGameCG ~= self.setting.skipStartGameCG then
    changedSetting.skipStartGameCG = self.setting.skipStartGameCG
    changed = true
  end
  self.oldSetting = nil
  if changed then
...
function FunctionPerformanceSetting:Apply(setting)
  ...
  if nil ~= setting.selfPeak and Game.Myself ~= nil then
...
function FunctionPerformanceSetting:GetSkipStartCG()
  return self.setting.skipStartGameCG
end
...
LanguageVoice = {Chinese = 1, Jananese = 2}
