...
function FunctionPerformanceSetting:ctor()
	self.setting = {
		...
		voiceLanguage = LanguageVoice.Jananese, --todo xde 强制日语
		powerMode = true,
		resolution = 1,
		showExterior = 0,
		selfPeak = true,
		otherPeak = true,
		push = 31, -- 这里为什么要写31而不是写0：在未登录的情况是不需要发推送的，防止服务器没法push的数据到客户端，客户端错误的认为用户已经全选了push，同理客户端在退到登录界面时应该将这个值重置
		gvoice = 0, -- 1队伍语音2公会语音4扬声器8麦克风
	}
