...
GuildFaithPage.LongPressEvent = "GuildFaithPage_LongPress"
...
function GuildFaithPage:InitUI()
	...
	--todo xde 
	local btnTitle = self:FindGO("Label", goAstrolabeButton):GetComponent(UILabel);
	OverseaHostHelper:FixLabelOverV1(btnTitle, 0, 200)
	...
	--todo xde 
	local sprite = self:FindGO("Sprite",self.lockTip):GetComponent(UISprite);
	OverseaHostHelper:FixAnchor(sprite.leftAnchor,self.lockTip.transform,1,-10)
	OverseaHostHelper:FixAnchor(sprite.rightAnchor,self.lockTip.transform,1,101)
	...
	local tabList = {self.faithTitle,self.astroTitle}
	for i,v in ipairs(tabList) do
		local longPress = v:GetComponent(UILongPress)
		longPress.pressEvent = function (obj, state)
			self:PassEvent(GuildFaithPage.LongPressEvent, {state, obj.gameObject});
		end
	end
	self:AddEventListener(GuildFaithPage.LongPressEvent, self.HandleLongPress, self)
...
function GuildFaithPage:HandleLongPress(param)
	...
	if not GameConfig.SystemForbid.TabNameTip then
		if isPressing then
			...
			TipManager.Instance:TryShowVerticalTabNameTip(GuildFaithPage.TabName[go.name],
					sp, NGUIUtil.AnchorSide.Down, {133, -58})
		else
