...
function PetAdventureHeadTip:FindObj()
	...
	--todo xde fix ui
	self.natureLabel = self:FindGO("Nature (1)"):GetComponent(UILabel);
	self.natureLabel.pivot = UIWidget.Pivot.Right
	self.friendly_valuelab.leftAnchor.target = self:FindGO("Friendly").transform
	self.friendly_valuelab.leftAnchor.absolute = 0
	self.friendly_valuelab.leftAnchor.relative = 1
	self:FindGO("FriendlySymbol"):SetActive(false)
	
end

function PetAdventureHeadTip:SetData(data)
	self.petEggData=data
	if(not self.petEggData)then return end 
	if(self.petEggData)then
		...
		if(self.petEggData.skillids and #self.petEggData.skillids>0)then
			local skilldatas = {};
