    ...
    if(SDKEnable) then
        ...
        if(passportInfoStr ~= "")then
            ...
            if(passPortInfo['platform_uid'] ~= nil)then
                ...
                local wg_method = "user.setrole" --固定
                local wg_game_code = "AAAPAA" --固定
                local wg_version = "1" --固定
                local wg_platform_uid = passPortInfo['platform_uid'] --龙城SDK返回的平台ID
                local server = FunctionLogin.Me():getCurServerData()
                local wg_server_code = server ~= nil and server.serverid or 0 --游戏服务器ID
                wg_server_code = wg_server_code .. roleInfo.sequence --拼上位置
                local wg_role_name = roleInfo.name --游戏角色名
                local wg_game_uid = roleInfo.id --游戏角色ID
                local wg_time = os.time() --当前时间戳(秒)
                local requestData = {
                ...
                local wg_sign = WeGameHelper:caculateSign(requestData,'wg_version') --需要计算得出
