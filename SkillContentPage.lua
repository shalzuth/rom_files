...
function SkillContentPage:AddViewListener()
	-- self:AddListenEvt(MyselfEvent.SkillPointChange,self.RefreshPoints)
...
function SkillContentPage:FindObjs()
	...
	self.comContentPanel:ResetAndUpdateAnchors()
	self.proContentPanel:ResetAndUpdateAnchors()
	-- print(self.contentPanel.height)
	if(ApplicationInfo.IsIphoneX()) then
		self.professGrid.transform.localPosition = Vector3(0,53,0)
	end
	...
	self.proContentScroll.onProgressStateChange = function (progress,state)
		self:ScrollStateChange(progress,state)
	end
	
	--todo xde fix ui 
	self:FindGO("ConfirmTip",self.bottomrightMines):SetActive(false)
	self:FindGO("ConfirmTipLabel",self.bottomrightMines):SetActive(false)
end
...
function SkillContentPage:ClickProfessBtn(cell)
	-- self:ScrollToProfess(cell,true)
end
...
function SkillContentPage:ScrollToProfess(index,spring)
	...
	if (not sv.canMoveHorizontally) then offset.x = self.proContentPanel.cachedTransform.localPosition.x end
	if (not sv.canMoveVertically) then offset.y = self.proContentPanel.cachedTransform.localPosition.y end
	if(spring) then
...
function SkillContentPage:RefreshProfess(note)
	...
	-- end
	-- self:SetScrollToProfess(myProfess)
end
...
function SkillContentPage:ShowProfess()
	...
	-- if(#self.professBtnList:GetCells() > 1) then
	-- 	self:Show(self.professBtnGrid.gameObject)
	-- else
	-- 	self:Hide(self.professBtnGrid.gameObject)
	-- end
	self:SetEditMode(false)
...
function SkillContentPage:LayOutProfessSkills(professTree)
	...
	for i=1,#cells do
		...
		if(professData == nil) then
			...
			professData = {minX=10000,maxX=0,id=cell.data.profession}
	...
	for i=1,#requiringCells do
		...
		-- print(cell.data.sortID,"?????? ??????",requiredSort)
...
function SkillContentPage:SetProgressScroll(gridXYs,max)
	...
	for i=1,#datas do
		...
		if professData then
			...
			self.professesSkillCenterPos[#self.professesSkillCenterPos + 1] = {pos = Vector3(x,0,0),minX = professData.minX,maxX = professData.maxX}
			-- self.proContentScroll:AddStateRange(i,professData.minX/max,professData.maxX/max,false)
...
function SkillContentPage:SetProfessBtns(professes)
	if(self.professDatas==nil) then
		...
		for i=1,#professes do
			self.professDatas[#self.professDatas + 1] = {data = professes[i],points = professes[i].points}
		end
	end
end
	...
	if(FunctionFirstTime.Me():IsFirstTime(FunctionFirstTime.SkillOverFlow)) then
		...
		if(equipedNum>=ShortCutProxy.Instance:GetUnLockSkillMaxIndex()) then
			...
			for i=1,#levelUpSkillIDs do
				if(Table_Skill[levelUpSkillIDs[i]].SkillType ~= GameConfig.SkillType.Passive.type) then
					if(skillProxy:GetEquipedSkillBySort(math.floor(levelUpSkillIDs[i]/1000))==false) then
						flag = true
						break
					end
				end
