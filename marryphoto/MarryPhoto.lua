...
function MarryPhoto:GetOriginImage(marry_id, pos_index, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback, is_through_marryphotocallback)
	...
	if isLatestLocal then
		...
		if is_through_marryphotocallback then
		...
		if not isDownloading then
			local downloadPath = Application.persistentDataPath .. '/' .. NetIngMarryPhoto.Ins():GetTempDownloadPathOfLocal(photoID, true)
			NetIngMarryPhoto.Ins():Download(
				marry_id,
				pos_index,
				photoID,
				timestamp,
...
function MarryPhoto:GetThumbnail(marry_id, pos_index, timestamp, progress_callback, success_callback, error_callback, is_keep_previous_callback)
	print(string.format('MarryPhoto:GetThumbnail\npos_index=%s\ntimestamp=%s\nis_keep_previous_callback=%s', tostring(pos_index), tostring(timestamp), tostring(is_keep_previous_callback)))
	local photoID = self:GetPhotoID(marry_id, pos_index)
	if not is_keep_previous_callback then
	...
	if isLatestLocal then
		...
		if not isDownloading and not self:IsGetingOForMakeT(photoID) and not self:IsMakingTWhenGetThumbnail(photoID) then
			local downloadPath = Application.persistentDataPath .. '/' .. NetIngMarryPhoto.Ins():GetTempDownloadPathOfLocal(photoID, false)
			NetIngMarryPhoto.Ins():Download(
				marry_id,
				pos_index,
				photoID,
...
function MarryPhoto:SaveAndUpload(marry_id, pos_index, bytes, timestamp, progress_callback, success_callback, error_callback)
	...
	if self:IsSaving(photoID) then
		...
		LocalMarryPhoto.Ins():SaveAsync(
			...
			timestamp,
			function ()
				local tempUploadRootPath = Application.persistentDataPath .. '/' .. NetIngMarryPhoto.Ins():GetTempUploadRootPathOfLocal()
				if not FileHelper.ExistDirectory(tempUploadRootPath) then
					FileHelper.CreateDirectory(tempUploadRootPath)
				end
				local tempUploadPathOfLocal = Application.persistentDataPath .. '/' .. NetIngMarryPhoto.Ins():GetTempUploadPathOfLocal(photoID)
...
function MarryPhoto:ClearLocal(marry_id, pos_index)
	local photoID = self:GetPhotoID(marry_id, pos_index)
	local downloadOPath = Application.persistentDataPath .. '/' .. NetIngMarryPhoto.Ins():GetTempDownloadPathOfLocal(photoID, true)
	if FileHelper.ExistFile(downloadOPath) then
		FileHelper.DeleteFile(downloadOPath)
	end
	local downloadTPath = Application.persistentDataPath .. '/' .. NetIngMarryPhoto.Ins():GetTempDownloadPathOfLocal(photoID, false)
