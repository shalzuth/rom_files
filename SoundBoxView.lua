...
function SoundBoxView:InitView()
	...
	--todo xde fix ui
	local label = self:FindGO("Label",self.chooseButton):GetComponent(UILabel)
	local spriteBtn = self.chooseButton:GetComponent(UISprite)
	OverseaHostHelper:FixAnchor(spriteBtn.leftAnchor,label.transform,0,-10)
	OverseaHostHelper:FixAnchor(spriteBtn.rightAnchor,label.transform,1,8)
	local noneTipLabel = self:FindGO("Label",self.noneTip):GetComponent(UILabel);
	OverseaHostHelper:FixLabelOverV1(noneTipLabel,3,260)
	local rightBg = self:FindGO("rightBg")
	local label = self:FindGO("Label",rightBg):GetComponent(UILabel)
	local label1 = self:FindGO("Label (1)",rightBg):GetComponent(UILabel)
	local label2 = self:FindGO("Label (2)",rightBg):GetComponent(UILabel)
	OverseaHostHelper:FixLabelOverV1(label,3,180)
	OverseaHostHelper:FixLabelOverV1(label1,3,180)
	OverseaHostHelper:FixLabelOverV1(label2,3,180)
