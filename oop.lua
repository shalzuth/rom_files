    ...
    return function (self, property)
		...
		if f~=nil then
			if(self.Alive and not self:Alive()) then
				LogUtility.InfoFormat("<color=red>class :{0} ????????????????????????????????????????????? key:{1}</color>", message_meta.__cname,tostring(property))
			end
			return f 
		end
		if(super) then
...
local function property_setter(message_meta,super)
	...
	return function (self, property, value)
		if(self.Alive and not self:Alive()) then
			LogUtility.InfoFormat("<color=red>class :{0} ??????????????????????????????????????????????????? key:{1} value:{2}</color>", message_meta.__cname,tostring(property), tostring(value))
		end
