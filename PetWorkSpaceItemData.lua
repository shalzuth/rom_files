...
function PetWorkSpaceItemData:GetDuringTime()
	...
	if(curTime<self.lastRewardTime)then
		helplog("??????,???????????????????????? ????????????????????? ",self.lastRewardTime,"?????????????????????: ",curTime)
		return 0
	end
...
function PetWorkSpaceItemData:IsOverCfgTime()
	...
	if(isOver)then
		-- helplog("IsOverCfgTime ???????????? ???",ClientTimeUtil.FormatTimeBySec(time))
	end
	return isOver
end
...
function PetWorkSpaceItemData:GetUIReward()
	...
	if(self.curCounts==nil or #self.curCounts==0)then
		if(multiple > 0  and #self.rewardArray>0)then
			...
			-- helplog("?????????????????? self.rewardArray[1].num: ",self.rewardArray[1].num,"??????????????????: ",multiple)
	...
	for i=1,#self.rewardArray do
		...
		if(not getCount)then
			-- helplog("?????????????????????",getCount,"???????????????",multiple)
		end
		-- helplog("????????????????????????",multiple,"getCount: ",getCount)
		local curMulti
		if(nilCount)then
		...
		if(curMulti>0)then
			...
			-- helplog("???????????????????????? ",getCount)
			-- helplog("?????????????????????self.rewardArray[i].num: ",self.rewardArray[i].num,"curMulti: ",curMulti)
